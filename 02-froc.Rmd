...
From our point of view the most important problem is that neglect of location information leads to loss of statistical power. The reason for this is the additional noise introduced into the AUC measurement due to crediting a radiologist for correctly detecting the diseased condition (a ROC paradigm True Positive or TP event) when the radiologist may have pointed to the wrong location, in which case *the radiologist is credited for a TP when in fact two cancelling mistakes were made: the true lesion was missed and a false location was incorrectly identified as diseased*. Not discriminating between TP events with correct localizations (associated with an expert radiologist) and those with incorrect localizations (a radiologist with lesser expertise) leads to lesser ability to distinguish between the two radiologists (their respective AUCs will be similar) and reduced statistical power when using the ROC paradigm as compared to the FROC paradigm described in this book.
