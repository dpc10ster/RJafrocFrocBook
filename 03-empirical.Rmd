...
### The inferred-ROC z-sample

The z-sample of the highest rated mark in a case, or $-\infty$ if the case has no marks, is defined as the inferred-ROC z-sample for the case. Inferred-ROC z-samples on non-diseased cases are referred to as inferred-FP z-samples and those on diseased cases as inferred-TP z-samples.
...
Consider a $R_{\text{FROC}}$ rating FROC study with allowed ratings $r = 1, 2, ..., R_{\text{FROC}}$. The ROC rating of a case is defined as the FROC rating of the highest rated actual mark on the case **plus 1** or 1 if the case has no marks. Since the 1 ROC rating is reserved for cases with no mark and the ratings are ordered labels, no ordering information is lost provided every other ROC rating is also "bumped up" by unity - this is the reason for the "plus one". The ROC rating scale therefore extends from $1$ to $R_{\text{FROC}} + 1$. Therefore, a $R_{\text{FROC}}$ rating FROC study corresponds to a $R_{\text{FROC}} + 1$ rating ROC study in the sense that both generate the same number $R_{\text{FROC}}$ of operating points.
...
knitr::kable(df, caption = "Summary of plots from FROC data. All empirical plots except FROC include a straight line extension from the uppermost observed point to (1,1).", escape = FALSE)
```
