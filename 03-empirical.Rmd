...
85%
FROC done; need to add text surrounding equations in Appendix 3



## TBA Introduction {#empirical-intro}

FROC data consists of mark-rating pairs. A distinction is made here between *latent* marks (suspicious regions perceived by the visual system) and *actual* marks. A key table, used in later chapters, summarizing FROC notation is introduced and the notation is used to define the empirical FROC plot (briefly introduced in the previous chapter). 


Operating characteristics are *visual* depictions of performance. Scalar measures derived from operating characteristics can serve as *quantitative* measures of performance. Empirical area measures (AUCs) associated with the FROC plot and other plots possible with FROC data are defined and illustrated with a small FROC dataset. Except for the FROC all of the other plots introduced in this chapter include a straight line extension from the uppermost observed operating point to (1,1).


If one ignores localization information and simply considers the highest rating on each case as representing its ROC rating, one can define the empirical ROC curve and associated area measure ROC-AUC from FROC data. Since ROC-AUC is a fundamental measure of classification accuracy between non-diseased and diseased cases any proposed other area measure that does not ignore location information should, if it is to be useful, correlate with ROC-AUC. These correlations are explored using the small dataset and it is shown that FROC-AUC is a poor measure of performance. While ways of circumventing FROC-AUC have been proposed and have been used by some investigators none are satisfactory and the claim of this book is that **the FROC should never be used to quantify performance**. The basic reason is simple: unlike all of the other operating characteristics defined in this chapter the FROC plot is not constrained to lie within the unit square and the area under a straight line extension to (1,1) is meaningless.


Some of the other empirical plots and AUCs are less familiar (as compared to the well-known ROC plots and ROC-AUC). As an aid to understanding them I have included numerical calculations of the empirical operating characteristics and AUCs for the small dataset. The calculations also illustrate the advantage of using weighted versions of some of the empirical plots (lesion weights are a way of allowing one to model the clinical importance (i.e., morbidity) of different type of lesions present in a clinical dataset; a weighted plot assures that each case gets the same importance in determining AUC regardless of the number of lesions in it). 


Computing the AUCs from plots can be tedious at best and computational formulae are needed which would allow any of the AUCs to be calculated directly from the FROC ratings. Appendix 1 proves a formula for one of the AUCs introduced in this chapter, Appendix 2 provides an interpretation of the area under the straight line extension of this. Appendix 3 summarizes, without proofs, the computation formulae for AUCs for all of the plots introduced in this chapter. 
...
fomTable = array(dim = c(6,3))
fomTable[1,]  <- c("FROC", "NLF", "LLF")
fomTable[2,]  <- c("ROC", "FPF", "TPF")
fomTable[3,]  <- c("AFROC", "FPF", "LLF")
fomTable[4,]  <- c("wAFROC", "FPF", "wLLF")
fomTable[5,]  <- c("AFROC1", "FPF1", "LLF")
fomTable[6,]  <- c("wAFROC1", "FPF1", "wLLF")
df <- as.data.frame(fomTable)
colnames(df) <- c("OCs", "X", "Y")
knitr::kable(df, caption = "Summary of operating characteristics (OCs) from FROC data; X = abscissa, Y = ordinate.", escape = FALSE)
```
...
Define the Wilcoxon kernel function $\psi(x,y)$ by:
...
\begin{equation}
\text{A}_{ROC} = \frac{1}{K_1K_2}  \sum_{k_2=1}^{K_2} \sum_{k_1=1}^{K_1} \psi\left (  r_{k_1} , r_{k_2} \right )
(\#eq:empirical-bamber-theorem)
\end{equation}


where $r_{k_1}$ and $r_{k_2}$ are the ROC ratings of non-diseased case $k_1$ and diseased case $k_2$ respectively.
...
$\text{A}_0$ equals the area of a rectangle with base $\left ( 1-\text{FPF}_1 \right )$ and height $\text{wLLF}_1$ plus the area of a triangle with base $\left ( 1-\text{FPF}_1 \right )$ and height $(1-\text{wLLF}_1)$, as shown next: 


\begin{equation}
\begin{aligned}
\text{A}_0 =& \left ( 1-\text{FPF}_1 \right )\text{wLLF}_1 
...
&\left[ \sum_{k_1=1}^{K_1}\sum_{l_1=1}^{N_{k_1 1}} \mathbb{I} \left ( z_{k_11l_11} \neq  -\infty \right ) \psi\left ( z_{k_11l_11},z_{k_22l_22} \right )+\sum_{k_2'=1}^{K_2}\sum_{l_1=1}^{N_{k_2' 2}} \mathbb{I} \left ( z_{k_11l_11} \neq  -\infty \right )\psi\left ( z_{k_2'2l_11},z_{k_22l_22} \right )
\right]
\end{aligned}
(\#eq:empirical-computational-froc)
\end{equation}

$\mathbb{I} \left ( z_{k_11l_11} \neq  -\infty \right )$ ensures that only *finite* NL z-samples enter the computation (recall that unmarked NLs are unobservable events). This conditioning is not needed for LLs since unmarked LLs are observable events. The first double summation inside the square bracket compares, using the $\psi$ function, all finite NL ratings on *non-diseased* cases $k_11$ with all lesion ratings on diseased case $k_22$. The second double summation compares all finite NL ratings on *diseased cases* $k_2'2$ with all lesion ratings on diseased case $k_22$. The double summation outside the square bracket sums over all diseased cases $k_22$ and all lesions in each diseased case. The final value is divided by the total number of cases and the total number of lesions.

The formula is equivalent to creating two arrays, the first containing all finite NL ratings and the second, with length equal to the total number of lesions, containing all lesion ratings including unmarked lesions. One compares, using the $\psi$ function, the NL and LL ratings in the two arrays and divides by the total number of cases and by the total number of lesions. 
