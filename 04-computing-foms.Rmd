...
Chapter `\@ref(froc-empirical)` introduced several empirical operating characteristic plots from FROC data. Expressions were given for computing *operating points* for each such plot from z-samples. The areas or AUCs under these plots were also defined. 
...
The highest rating $\text{FP}_{k_1 1}$ on non-diseased case $k_1 1$ was defined in Section \@ref(eq:froc-empirical-FP). The function $\psi\left ( x,  y \right )$ was defined (see chapter TBA on empirical AUC in `RJafrocRocBook`) by:
...
The empirical weighted-AFROC plot and lesion weights were defined in Section TBA `\@ref(froc-empirical-wAFROC)`. The corresponding AUC can be computed [@RN1385] by including the lesion weights $W_{k_2 l_2}$ inside the summations (but outside the kernel function):
