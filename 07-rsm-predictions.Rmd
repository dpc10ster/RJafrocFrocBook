...
In this chapter formulae for RSM quantities are given in terms of the RSM parameters $\lambda$ and $\nu$. The formulae can be transformed to intrinsic RSM parameters $\lambda_i$ and $\nu_i$ using Eqn. \@ref(eq:rsm-inv-transform). 



## Inferred ROC z-sample {#rsm-pred-inferred-roc}

*The inferred ROC z-sample of a case, denoted $h_{k_t t}$, is the z-sample of the highest rated latent mark on the case or $-\infty$ if the case has no latent marks.* For non-diseased cases the maximum is over all latent NLs on the case. For diseased cases the maximum is over all latent NLs *and* latent LLs on the case.  
...
As another way of describing this unusual behavior, as the observer is encouraged to be more "aggressive in reporting lesions", the ROC point moves continuously upwards and to the right from (0, 0) to the end-point, $\left (\text{FPF}_{\text{max}}, \text{TPF}_{\text{max}} \right )$, and no further. The ROC curve cannot just “hang there” since cumulating all cases always yields the (1,1) operating point. **Therefore, the complete ROC curve is obtained by extending the end-point using a dashed line that connects it to (1,1).** The observer cannot operate along the dashed line. In the language of "moving up the ROC curve" there is a discontinuous jump from the end-point to (1,1). At the end-point the shape of the ROC curve changes from concave-down to a dashed straight line. It can be shown TBA that the limiting slope of the continuous ROC at the end-point is equal to the slope of the dashed straight line connecting the end-point to (1,1). 
 
How closely the operating point approaches the limiting point $\left (\text{FPF}_{\text{max}}, \text{TPF}_{\text{max}} \right )$ is unrelated to the number of bins; rather, it depends on $\zeta_1$. As the latter is lowered the observed end-point approaches $\left (\text{FPF}_{\text{max}}, \text{TPF}_{\text{max}} \right )$ from below-left. As will be shown below, how closely $\left (\text{FPF}_{\text{max}}, \text{TPF}_{\text{max}} \right )$ approaches (1,1) depends on the $\lambda'$ and $\nu'$ RSM parameters: namely, as $\lambda'$ and $\nu'$ increase, $\left (\text{FPF}_{\text{max}}, \text{TPF}_{\text{max}} \right )$  approaches (1,1) from below-left. 
...
Therefore the probability that the case has *at least one* latent NL is the complement of the above probability. At sufficiently low $\zeta_1$ each of these cases yields a FP. Therefore, the maximum continuously accessible abscissa of the ROC, i.e., $\text{FPF}_{\text{max}}$, is: 
...
Therefore, the probability that there exists *at least one* latent site is the complement of the above expression, which equals $\text{TPF}_{\text{max}}$, i.e., 
...
The ordinate of the end-point is a weighted summation of $\text{TPF}_{\text{max}} \left ( \mu, \lambda', \nu', L \right )$ over the lesion distribution vector $\overrightarrow{f_L}$. 

The expression for $\text{FPF}_{\text{max}}$ is unaffected.
...
Varying the threshold parameter $\zeta$ from $\infty$ to $-\infty$ sweeps out the continuous section of the predicted ROC curve from (0,0) to $\left (\text{FPF}_{\text{max}}, \text{TPF}_{\text{max}} \right )$. 
...
The conditioning notation in Eqn. \@ref(eq:rsm-pred-fpf-zeta-n) reflects the fact that this expression applies specifically to non-diseased cases with $n$ latent NLs each. To obtain $\text{FPF}_{\text{max}}$ one performs a Poisson-weighted summation of $\text{FPF}\left (\zeta \mid n \right )$ over $n$ from 0 to $\infty$ (the inclusion of the $n = 0$ term is explained below):    
...
```{r rsm-pred-roc-zeta1, fig.cap="ROC curves for two values of $\\zeta_1$: both curves correspond to $\\mu = 2$, $\\nu = 1$ and $\\lambda = 1$. The red curve corresponds to $\\zeta_1 = -10$ and the blue curve to $\\zeta_1 = 1$.", fig.show='hold', echo=FALSE}
...
The end point coordinates of the continuous part of ROC curve was derived above, Eqn. \@ref(eq:rsm-pred-fpf-max) for $\text{FPF}_{\text{max}}$ and Eqn. \@ref(eq:rsm-pred-tpf-max) for $\text{TPF}_{\text{max}}$. Therefore, the slope $m_{st}$ of the dashed dashed straight line is:
