...
The `FALSE` value corresponds to $\mu = 0.001$, which is likely a numerical issue associated with commparing very small values. The remaining $\mu$ values yield `TRUE`.
...
For equal weights the following code tests if wAFROC `wLLF` values are less than or equal to the corresponding AFROC `LLF` values.
