...
```{r rsm-pred2-froc-plots1, fig.cap="RSM-predicted FROC curves in terms of intrinsic parameters $\\lambda_i = 2$ and $\\nu_i = 0.5$. Top left: $\\mu = 0.0001$; Top right: $\\mu = 1$; Bottom left: $\\mu = 2$; Bottom right: $\\mu = 4$.", fig.show='hold', echo=FALSE}
suppressWarnings(grid.arrange(plotArr[[1]],plotArr[[2]],plotArr[[3]],plotArr[[4]], ncol = 2))
```
...
```{r rsm-pred2-afroc-plots-code1, echo=FALSE}
plotArrAfroc <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  mu <- muArr[i]
  ret <- UtilIntrinsic2RSM(mu, lambda_i = lambda_i, nu_i = nu_i)
