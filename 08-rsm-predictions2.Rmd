...
```{r rsm-pred2-froc-plots-lambdai-nui, fig.cap="RSM-predicted FROC curves using intrinsic parameters $\\lambda_i = 2$ and $\\nu_i = 0.5$. Top left: $\\mu = 0.0001$; Top right: $\\mu = 1$; Bottom left: $\\mu = 2$; Bottom right: $\\mu = 4$. Each plot is **not** contained within the unit square which makes it impossible to define the AUC as a figure of merit.", fig.show='hold', echo=TRUE}
...
plotArr_AFROC_lambdai_nui <- array(list(), dim = c(length(muArr)))
for (i in 1:length(muArr)) {
  mu <- muArr[i]
  ret <- UtilIntrinsic2RSM(mu, lambda_i = lambda_i, nu_i = nu_i)
  lambda <- ret$lambda  
  ...
  plotArr_AFROC_lambdai_nui[[i]] <- ret1$AFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```
...
grid.arrange(plotArr_AFROC_lambdai_nui[[1]],plotArr_AFROC_lambdai_nui[[2]],plotArr_AFROC_lambdai_nui[[3]],plotArr_AFROC_lambdai_nui[[4]], ncol = 2)
```
...
grid.arrange(plotArr_wAFROC_lambdai_nui[[1]],plotArr_wAFROC_lambdai_nui[[2]],plotArr_wAFROC_lambdai_nui[[3]],plotArr_wAFROC_lambdai_nui[[4]], ncol = 2)
```
...
plotArr_FROC_lambda_nu <- array(list(), dim = c(length(muArr)))
for (i in 1:length(muArr)) {
  mu <- muArr[i]
  lambda <- lambdaArr[i]
  nu <- nuArr[i]
  ...
  plotArr_FROC_lambda_nu[[i]] <- ret1$FROCPlot 
  #+ ggtitle(paste0("mu = ", as.character(muArr[i])))
}
```
...
grid.arrange(plotArr_FROC_lambda_nu[[1]],plotArr_FROC_lambda_nu[[2]],plotArr_FROC_lambda_nu[[3]],plotArr_FROC_lambda_nu[[4]], ncol = 2)
```
...
plotArr_AFROC_lambda_nu <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  ...
  plotArr_AFROC_lambda_nu[[i]] <- ret1$AFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```
...
grid.arrange(plotArr_AFROC_lambda_nu[[1]],plotArr_AFROC_lambda_nu[[2]],plotArr_AFROC_lambda_nu[[3]],plotArr_AFROC_lambda_nu[[4]], ncol = 2)
```
...
plotArr_wAFROC_lambda_nu <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  mu <- muArr[i]
  lambda <- lambdaArr[i]
  nu <- nuArr[i]
  ...
  plotArr_wAFROC_lambda_nu[[i]] <- ret1$wAFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```
...
grid.arrange(plotArr_wAFROC_lambda_nu[[1]],plotArr_wAFROC_lambda_nu[[2]],plotArr_wAFROC_lambda_nu[[3]],plotArr_wAFROC_lambda_nu[[4]], ncol = 2)
```
