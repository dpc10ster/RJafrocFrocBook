...
* $\mathbf{W}$ is the (lower triangular) square weights matrix with $L_{max}$ rows and columns, where each row (excluding cells set to $-\infty$) sums to unity, see example below (the unused matrix elements are set to $-\infty$). In Eqn. \@ref(eq:rsm-pred2-wllf) the index $l_2$ in $W_{Ll_2}$ ranges from 1 to L. 
...
### ROC curves are generally above the AFROC curves {#rsm-pred2-roc-above-afroc}


Using previous equations for the ROC TPF and the AFROC LLF, one has:
...
* The equality is achieved when $\zeta = +\infty$, i.e., at the origin, or if $\mu = \infty$ (since in either case the $\Phi$ function evaluates to zero).



### wAFROC curves are generally above the AFROC curves except for small $\mu$ {#rsm-pred2-wafroc-above-afroc}
...
```{r echo=T}
for (i in 1:4) {
  comparison <- all(p_wAFROC_lambda_nu[[i]]$data$wLLF 
...
The `FALSE` value corresponds to very small $\mu = 0.1$. The remaining $\mu$ values yield `TRUE`.


### Equal weight wAFROC curves are identical to AFROC curves {#rsm-pred2-wafroc-above-afroc-zero-weights}
...
* In the following code we set `relWeights = 0` to ensure equal weighting of all lesions. In `p_wAfroc0` the `Zero` refers to zero weights.


```{r , echo=TRUE}
p_wAfroc0 <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  mu <- muArr[i]
  lambda <- lambdaArr[i]
  nu <- nuArr[i]
  ...
  p_wAfroc0[[i]] <- ret1$wAFROCPlot
}
```



In the following code we test numerically, for each value of $\mu$, if *all of the absolute values of the differences between the two arrays* are smaller than $1e-07$.


```{r echo=T}
for (i in 1:4) {
  comparison <- all(abs(p_wAfroc0[[i]]$data$wLLF - 
                          p_AFROC_lambda_nu[[i]]$data$LLF) < 1e-7)
  cat(sprintf("mu = %6.3f, comparison = %5s", muArr[i], comparison), "\n")
}
```
