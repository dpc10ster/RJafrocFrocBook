...
The clear connection between $\lambda$ and $\nu$ and the FROC end-point is the reason they are called the *physical* RSM parameters. For reasons explained in Section \@ref(froc-paradigm-solar-analogy) the physical parameters are not the best way of thinking about predicted RSM curves. Basically they hide an inherent $\mu$ dependence - ignoring this can lead to poor choices of RSM parameters as shown in Appendix \@ref(rsm-pred2-appendix-slope-froc). Intrinsic parameters were introduced in Section \@ref(rsm-intrinsic-parameters) which takes into account this dependence. For convenience the transformations between physical and intrinsic parameters are repeated here:
...
The following code shows how to generate FROC plots using the intrinsic $\lambda_i = 2$ and $\nu_i = 0.5$ parameters for 4 values of $\mu$ contained in the array `muArr <- c(0.0001,1,2,4)` (to avoid a divide by zero error the value $\mu=0$ is not allowed). A `list` array `plotArr_FROC_lambdai_nui` is created to contain the four plots. The intrinsic intrinsic $\lambda_i, \nu_i$ parameters are converted to $\lambda, \nu$ using the function `UtilIntrinsic2RSM()`which implements Eqn. \@ref(eq:rsm-pred2-transform)). The parameters are printed using the `cat()` function. The plots are generated using `PlotRsmOperatingCharacteristics()`. Online help on this function is [available](https://dpc10ster.github.io/RJafroc/reference/PlotRsmOperatingCharacteristics.html). The code `plotArr_FROC_lambdai_nui[[i]] <- ret1$FROCPlot` saves the plot to the previously created `list` array. 
...
```{r rsm-pred2-froc-plots-lambdai-nui, fig.cap="RSM-predicted FROC curves using intrinsic parameters $\\lambda_i = 2$ and $\\nu_i = 0.5$. Top left: $\\mu = 0.0001$; Top right: $\\mu = 1$; Bottom left: $\\mu = 2$; Bottom right: $\\mu = 4$. Each plot is **not** contained within the unit square which makes it impossible to define the FROC-AUC as a figure of merit.", fig.show='hold', echo=TRUE}
suppressWarnings(grid.arrange(
plotArr_FROC_lambdai_nui[[1]],
...
> 
* In the top-left plot, because $\lambda = 2000$ defines the end-point abscissa and $\nu = 5 \times 10^{-5}$ defines the end-point ordinate, the FROC curve hugs the x-axis approaching $(2000, 5 \times 10^{-5})$. This is the chance line FROC for very small $\mu$. 
* Recall the solar analogy in Section \@ref(froc-paradigm-solar-analogy). When lesion contrast is very low the search mechanism has little chance of finding lesions (leading to small LLF) and generates large numbers of NLs in attempting to do so (leading to large NLF). 
...
The AFROC x-coordinate is the same as the ROC x-coordinate and Eqn. \@ref(eq:rsm-pred-fpf) applies. The AFROC y-coordinate is identical to the FROC y-coordinate and Eqn. \@ref(eq:rsm-pred2-llf) applies. Like the FROC the AFROC is independent of the number of lesions in the dataset or their distribution. 
...
### Slope of the FROC curve {#rsm-pred2-appendix-slope-froc}
...
Eqn. \@ref(eq:rsm-pred2-froc-slope) leads to the following conclusions (recall $\mu \ge 0$):
...
If instead we had used Eqn. \@ref(eq:rsm-pred2-froc-slope1) and regarded $\lambda$ and $\nu$ as independent of $\mu$, the last conclusion would change to: 
...
The AFROC ordinate is LLF and the abscissa is FPF. Expressions for both were given above. Taking the derivatives of these functions with respect to $\zeta$ the slope of the AFROC is given by:
...
The numerator is the slope of the FROC, Eqn. \@ref(eq:rsm-pred2-froc-slope). Since $0 \le \text{FPF} \le \text{FPF}_{\text{max}}$ and $\text{FPF}$ increases as $\zeta$ decreases, the slope of the AFROC equals that of the FROC at the origin and subsequently increases over that of the FROC as the end-point is approached. 

This expression leads to the following conclusions (using the intrisic parameters):


>
* The slope of the AFROC near the end-point, corresponding to $\zeta = -\infty$, is zero provided $\mu \ne 0$. 
* The slope near the origin, corresponding to $\zeta = +\infty$, is $\infty$ provided $\mu \ne 0$.
* For $\mu = 0$ the slope of the AFROC is zero regardless of the value of $\zeta$, see top-left panel in Fig. \@ref(fig:rsm-pred2-froc-plots-lambdai-nui). 


Using the physical parameters the last conclusion changes to an unreasonable one:
