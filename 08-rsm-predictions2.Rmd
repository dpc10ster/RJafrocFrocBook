...
Like the ROC the AFROC has the constrained end-point property (i.e., the end-point lies within the unit square). 
...
```{r rsm-other-predictions-afroc-plots-lambda-nu, fig.cap="RSM-predicted AFROC curves  using $\\lambda, \\nu$ paramterization. Top left: $\\mu = 0.1$, $\\lambda = 1$ and $\\nu = 0.2$. Top right: $\\mu = 1$, $\\lambda = 2$ and $\\nu = 0.5$. Bottom left: $\\mu = 2$, $\\lambda = 3$ and $\\nu = 0.7$. Bottom right: $\\mu = 4$, $\\lambda = 4$ and $\\nu = 0.9$. As $\\mu$ increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1). Since each plot is contained within the unit square its AUC is a valid figure of merit. The top-left panel is an unrealistic prediction because of the supplied unrealistic parameters $\\lambda =1, \\nu = 0.2$ for small $\\mu$.", fig.show='hold', echo=FALSE}
...
```{r rsm-other-predictions-wafroc-plots-lambda-nu, fig.cap="RSM-predicted wAFROC curves using $\\lambda, \\nu$ paramterization. Top left: $\\mu = 0.1$, $\\lambda = 1$ and $\\nu = 0.2$. Top right: $\\mu = 1$, $\\lambda = 2$ and $\\nu = 0.5$. Bottom left: $\\mu = 2$, $\\lambda = 3$ and $\\nu = 0.7$. Bottom right: $\\mu = 4$, $\\lambda = 4$ and $\\nu = 0.9$. As $\\mu$ increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1). Since each plot is contained within the unit square its AUC is a valid figure of merit.", fig.show='hold', echo=FALSE}
