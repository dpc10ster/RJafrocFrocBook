...
### FROC plots using $\lambda_i, \nu_i$ parameters

TBA A few FROC plots are shown below which illustrate the statements just made in the previous section.
...
```{r rsm-pred2-froc-plots-lambdai-nui, fig.cap="RSM-predicted FROC curves using intrinsic parameters $\\lambda_i = 2$ and $\\nu_i = 0.5$. Top left: $\\mu = 0.0001$; Top right: $\\mu = 1$; Bottom left: $\\mu = 2$; Bottom right: $\\mu = 4$.", fig.show='hold', echo=FALSE}
suppressWarnings(grid.arrange(plotArr_FROC_lambdai_nui[[1]],plotArr_FROC_lambdai_nui[[2]],plotArr_FROC_lambdai_nui[[3]],plotArr_FROC_lambdai_nui[[4]], ncol = 2))
```
...
```{r rsm-pred2-wafroc-plots-lambdai-nui-code, echo=FALSE}
...
for (i in 1:length(muArr)) {
  ...
  plotArr_wAFROC_lambdai_nui[[i]] <- ret1$wAFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```
...
### AFROC plots using $\lambda, \nu$ parameters


>
* Shown below are AFROC curves for the same parameter values used to demonstrate the FROC curves shown in Fig. \@ref(fig:rsm-pred2-froc-plots-lambda-nu). 
...
* Notice that the AFROC curve is concave-up for very small value of $\mu$, see top-left plot in Fig. \@ref(fig:rsm-pred2-afroc-plots-lambda-nu). For technical reasons I have used a small non-zero value $\mu = 0.0001$ as $\mu = 0$ will generate an error message in the software.  
...
>
* Shown below are wAFROC curves for the same parameter values used to demonstrate the FROC curves shown in Fig. \@ref(fig:rsm-pred2-froc-plots-lambda-nu). 
