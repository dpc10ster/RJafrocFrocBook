...
```{r rsm-pred2-froc-plots-code, echo=FALSE}
muArr <- c(0.0001,1,2,4)
lambdaArr <- c(1,2,3,4)
nuArr <- c(0.2,0.5,0.7,0.9)
plotArr <- array(list(), dim = c(length(lambdaArr)))
...
```{r rsm-pred2-afroc-plots-code, echo=FALSE}
plotArrAfroc <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  mu <- muArr[i]
  lambda <- lambdaArr[i]
  nu <- nuArr[i]
  ...
  plotArrAfroc[[i]] <- ret1$AFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```



```{r rsm-pred2-afroc-plots, fig.cap="RSM-predicted AFROC curves for the same parameter values used in the earlier FROC plot. Top left: $\\mu = 0.0001$, $\\lambda = 1$ and $\\nu = 0.2$. Top right: $\\mu = 1$, $\\lambda = 2$ and $\\nu = 0.5$. Bottom left: $\\mu = 2$, $\\lambda = 3$ and $\\nu = 0.7$. Bottom right: $\\mu = 4$, $\\lambda = 4$ and $\\nu = 0.9$. As $\\mu$ increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit.", fig.show='hold', echo=FALSE}
grid.arrange(plotArrAfroc[[1]],plotArrAfroc[[2]],plotArrAfroc[[3]],plotArrAfroc[[4]], ncol = 2)
```
...
* Note that each wAFROC curve needs to be completed by a straight line extending from the end-point to (1,1). This line is inaccessible to the observer (except for the (1,1) point trivially obtained by cumulating all cases, including those with no marks) but is necessary to fully account for all decisions (an analogous situation was encountered in connection with the RSM predicted ROC curve). 
* Notice that the wAFROC curve is concave-up for very small value of $\mu$, see top-left plot in Fig. \@ref(fig:rsm-pred2-afroc-plots). For technical reasons I have used a small non-zero value $\mu = 0.0001$ as $\mu = 0$ will generate an error message in the software.  
* Each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit.
* As $\mu$ increases, the area under the wAFROC (including that under the straight line extension) increases monotonically from 0 to 1. The reader should check that this is true regardless of the choices of the other parameters in the model. This is expected of a well-behaved area measure that can be used as a figure of merit. 


```{r rsm-pred2-wafroc-plots-code, echo=FALSE}
plotArrwAfroc <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  mu <- muArr[i]
  lambda <- lambdaArr[i]
  nu <- nuArr[i]
  ...
  plotArrwAfroc[[i]] <- ret1$wAFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```



```{r rsm-pred2-wafroc-plots, fig.cap="RSM-predicted AFROC curves for the same parameter values used in the earlier FROC plot. Top left: $\\mu = 0.0001$, $\\lambda = 1$ and $\\nu = 0.2$. Top right: $\\mu = 1$, $\\lambda = 2$ and $\\nu = 0.5$. Bottom left: $\\mu = 2$, $\\lambda = 3$ and $\\nu = 0.7$. Bottom right: $\\mu = 4$, $\\lambda = 4$ and $\\nu = 0.9$. As $\\mu$ increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit.", fig.show='hold', echo=FALSE}
grid.arrange(plotArrwAfroc[[1]],plotArrwAfroc[[2]],plotArrwAfroc[[3]],plotArrwAfroc[[4]], ncol = 2)
```
...
If one sets `relWeights = 0`, as in the following code, thereby ensuring equal weights of all lesions, then the wAFROC and AFROC values are identical.
...
for (i in 1:length(lambdaArr)) {
  ...
  plotArrwAfrocZero[[i]] <- ret1$wAFROCPlot #+ ggtitle(paste0("mu = ", as.character(muArr[i]), ", AUC = ", format(ret1$aucAFROC, digits = 3)))
}
```
...
Testing for exact equality does not yield `TRUE` due to numerical issues.
