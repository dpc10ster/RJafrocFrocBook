...
The RSM, PROPROC and CBM algorithms were applied to the 14 datasets described in Chapter \@ref(datasets). The analyzed results file locations are shown in Section \@ref(rsm-3-fits-pre-analyzed-results).
...
* The examples use the function `Compare3ProperRocFits()`. 
* The code file is `R/compare-3-fits/Compare3ProperRocFits.R`. 
* In the current example two datasets are analyzed, `datasetNames[2]` and `datasetNames[3]`, i.e., the `r datasetNames[2]` and `r datasetNames[3]` datasets.  
...
The plot is labeled `D2, i = 1, j = 2`, meaning the second dataset, the second treatment and the second reader. It contains 3 curves:
...
CBM has parameters $\mu_{CBM}$, $\alpha$ and $\zeta_1$.

The RSM has parameters: $\mu$, $\lambda'$, $\nu'$ and $\zeta_1$. The parameters corresponding to the RSM plots are accessed as shown next. 

* `resultsArr[[f]][[i,j]]$retCbm$mu` is the CBM $\mu$ parameter, 
* `resultsArr[[f]][[i,j]]$retCbm$alpha` is the CBM $\alpha$ parameter, and  
