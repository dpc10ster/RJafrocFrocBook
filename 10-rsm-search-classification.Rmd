...
The preceding two chapters described predictions of the radiological search model (RSM). This chapter describes two performance metrics, namely *search and lesion-classification performances*, that can be derived from the end-point of the predicted ROC curve. These terms were introduced in Section \@ref(visual-search-search-classification).

Recall the search involves two stages: (1) finding suspicious regions and (2) correctly classifying each suspicious region as either a lesion (which is marked and rated) or a non-lesion (which is not marked). The first stage is the search task while the second stage is the lesion-classification task. Performance in the search task is measured by the ability to find lesions while minimizing finding non-lesions. Performance in the classification task is the ability, having found a suspicious region, to correctly recognize it as a lesion (to be marked and rated) or a non-lesion. The aim of this chapter is to quantify these two abilities. It turns out that all that is needed to accomplish this aim is the end-point of the ROC.

We start by recapitulating the formulae for the location of the RSM-predicted end-point. 

## Location of ROC end-point {#rsm-search-classification-end-point}
...
## Quantifying search performance {#rsm-search-classification-quantifying}
...
We have shown that the RSM parameters determine search and lesion classification performance. Assuming these parameters can be estimated the results of this chapter should be of interest to researchers in the area of computer aided detection -- CAD -- algorithm design, because they yield information about which stage -- search or lesion classification -- is limiting performance. If search performance is low then the observer is having difficulty finding lesions while minimizing finding non-lesions. In the CAD context, the *initial detection* stage needs to be further optimized. If lesion classification performance is low the observer is finding lesions efficiently but is having difficulty correctly classifying the found lesions.  In the CAD context, the *candidate analysis* stage needs to be further optimized. Of course, for this to be realized one needs a method for estimating the RSM parameters. This is the subject of the next chapter.
