...
ndTable[1,]  <- c("1", "1111", "$-\\infty$", "$-\\infty$", "")
ndTable[2,]  <- c("1", "1121", "$-\\infty$", "$-\\infty$", "")
ndTable[3,]  <- c("2", "2111", "0.487",  "", "")
ndTable[4,]  <- c("2", "2121", "$-\\infty$",  "0.487", "F")
ndTable[5,]  <- c("3", "3111", "0.738",  "", "")
ndTable[6,]  <- c("3", "3121", "0.576",  "0.738", "E")
ndTable[7,]  <- c("4", "4111", "-0.305", "", "")
ndTable[8,]  <- c("4", "4121", "$-\\infty$",  "-0.305", "H")
df <- as.data.frame(ndTable)
colnames(df) <- c("", "$k_t t l_s s$", "$z_{k_t t l_s s}$", "$FP_{k_t t}$", "$\\text{Label}$")
knitr::kable(df, caption = "Layout of mark-rating  pairs on non-diseased cases.", escape = FALSE) %>% collapse_rows(columns = c(1,3,4), valign = "middle") %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c") 
```
...
disTable[3,]  <- c("2", "1", "2211", "", "2212", "-0.215", "1", "G")
disTable[4,]  <- c("2", "1", "2221", "", "2222", "", "",  "")

disTable[5,]  <- c("3", "2", "3211", "", "3212", "1.588", "0.6", "C")
disTable[6,]  <- c("3", "2", "3221", "", "3222", "", "0.4", "UM")
disTable[7,]  <- c("4", "2", "4211", "", "4212", "2.944", "0.4", "A")
...
colnames(df) <- c("",
                  ...
                  "$\\text{weights}$",
                  "$\\text{Label}$")
knitr::kable(df, caption = "Layout of mark-rating pairs on diseased cases; UM denotes an unmarked lesion.", escape = FALSE) %>% collapse_rows(columns = c(1), valign = "middle") %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c")
```
