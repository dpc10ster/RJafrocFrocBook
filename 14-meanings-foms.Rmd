...
p <- ggplot(wAFROC_Points, aes(x = FPF, y = wLLF) ) + geom_polygon(data = shade1, fill = 'grey') + geom_polygon(data = shade2, fill = 'grey') + 
  ...
  annotation_custom(grob = textGrob(bquote(italic(wLLF["1"])),
                    ...
                    xmin = 0.87, xmax = 0.87, 
                    ymin = 0.47, ymax = 0.47) +
  geom_segment(aes(x = 0.85, y = 0.5, xend = 0.75, yend = 0.1), arrow = arrow(angle = 20, length = unit(0.5, "cm"))) + 
...
```{r froc-meanings-numerical, echo = F, cache=F}
...
Shown next is the `NL` ratings array. The array has 8 rows, corresponding to the total number of cases, and 2 columns, corresponding to the maximum number of NLs per case (notice there are two entries for case #3). The negative infinities represent unmarked locations. 
...
These values can be used to numerically calculate the AUCs under the AFROC and wAFROC.

```{r}
afroc_auc <- 0.5 * 0.667 + 0.25 * 0.833 + 
  0.833 * 0.25 +  (1 - 0.833) * 0.25 /2
...
Shown in Fig. \@ref(fig:plots-afrocPlot-wafrocPlot) are the AFROC and wAFROC plots with operating points.
...
```{r linearPlotAFROC, echo = FALSE, cache=F}
LL <- frocData$ratings$LL[(frocData$lesions$weights != -Inf)]
ratings <- c(FP,	LL)
...
linearPlotAFROC <- ggplot(data = ret) + ggtitle ("A") + 
  geom_point(aes(x = ratings, y = pos, color = factor(truth)), size = 6) + 
...
```{r linearPlotwAFROC, echo=FALSE, cache=FALSE}
...
frocData$lesions$weights[3, ] <- c(0.1, 0.9)
frocData$lesions$weights[4, ] <- c(0.9, 0.1)
weight <- c(rep(1, length(FP)), frocData$lesions$weights[frocData$lesions$weights != -Inf])
ret <- data.frame(ratings = ratings, truth = truth, weight = weight, pos = pos)
ret$ratings[ret$ratings == -Inf] <- -0.9
z_text <- data.frame(x = 3.5, y = -0.09, label = "z")
...
linearPlotwAFROC <- ggplot(data = ret) + ggtitle("B") + 
  ...
  scale_color_manual(values = c("green", "red")) +
  scale_size_manual(values = c(3, 1, 4) + 2) + 
  theme_bw() + 
  ...
  annotation_custom(grob = textGrob(bquote(z[3222]),
                    ...
                    xmin = -0.68, xmax = -0.68, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[32]==0.1),
                                    gp = gpar(fontsize = 8)), 
                    xmin = -0.68, xmax = -0.68, 
                    ymin = -0.36, ymax = -0.36) +
  ...
  annotation_custom(grob = textGrob(bquote(z[3212]),
                    ...
                    xmin = 1.43, xmax = 1.43, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[31]==0.9),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 1.43, xmax = 1.43, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = 2.12, y = -0.25, xend = 2, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[4222]),
                    ...
                    xmin = 2.12, xmax = 2.12, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[42]==0.9),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 2.12, xmax = 2.12, 
                    ymin = -0.36, ymax = -0.36) +
  geom_segment(aes(x = 2.78, y = -0.25, xend = 2.9, yend = -0.08), arrow = arrow(length = unit(0.2, "cm"))) +
  annotation_custom(grob = textGrob(bquote(z[4212]),
                    ...
                    xmin = 2.78, xmax = 2.78, 
                    ymin = -0.29, ymax = -0.29) +
  annotation_custom(grob = textGrob(bquote(W[41]==0.1),
                                    gp = gpar(fontsize = 8)), 
                    xmin = 2.78, xmax = 2.78, 
                    ymin = -0.36, ymax = -0.36)
...
```{r afrocPlot, echo=FALSE, cache=F}
LL <- frocData$ratings$LL[(frocData$lesions$weights != -Inf)]
ratings <- c(FP,	LL)
truth <- c(rep(0, length(FP)), rep(1, length(LL)))
weight <- rep(1, length(ratings))
...
frocData$lesions$weights[3, ] <- c(0.9, 0.1)
frocData$lesions$weights[4, ] <- c(0.1, 0.9)
weight <- c(rep(1, length(FP)), frocData$lesions$weights[frocData$lesions$weights != -Inf])
ret <- data.frame(ratings = ratings, truth = truth, weight = weight, pos = pos)
ret$ratings[ret$ratings == -Inf] <- -0.9
z_text <- data.frame(x = 3.5, y = -0.09, label = "z")
...
```{r wafrocPlot, echo = FALSE, cache=F}
FP <- FP[1:K1]
LL <- frocData$ratings$LL[(frocData$lesions$weights != -Inf)]
ratings <- c(FP,	LL)
truth <- c(rep(0, length(FP)), rep(1, length(LL)))
