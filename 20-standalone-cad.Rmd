...
dig <- 3
ciRad <- paste0("(", 
                format(x$CIAvgRadFom[1], scientific = TF, digits = dig),
                ",",
                format(x$CIAvgRadFom[2], scientific = TF, digits = dig),
                ")")

ciDif <- paste0("(", 
                format(x$CIAvgDiffFom[1], scientific = TF, digits = dig),
                ",",
                format(x$CIAvgDiffFom[2], scientific = TF, digits = dig)
                ,")")
...
ret <- c(
  ...
  ciDif, # ci DIFF
  format(as.numeric(x$Tstat)^2, scientific = TF, digits = dig),  
  format(as.numeric(x$df), scientific = TF, digits = dig),  
  format(as.numeric(x$pval), scientific = TF, digits = dig))  
...
dig <- 3
ciCad <- paste0("(", 
                format(x$ciAvgRdrEachTrt$CILower[1], scientific = TF, digits = dig),
                ",",
                format(x$ciAvgRdrEachTrt$CIUpper[1], scientific = TF, digits = dig),
                ")")
ciRad <- paste0("(", 
                format(x$ciAvgRdrEachTrt$CILower[2], scientific = TF, digits = dig),
                ",",
                format(x$ciAvgRdrEachTrt$CIUpper[2], scientific = TF, digits = dig),
                ")")
ciDif <- paste0("(", 
                format(x$ciDiffFom$CILower, scientific = TF, digits = dig), 
                ",",
                format(x$ciDiffFom$CIUpper, scientific = TF, digits = dig),
                ")")
...
ret <- c(
  ...
  format(x$avgDiffFom, scientific = TF, digits = dig),
  ciDif, # ci DIFF
  format(x$FStat, scientific = TF, digits = dig),  
  format(x$df, scientific = TF, digits = dig),  
...
dig <- 3
ciRad <- paste0("(", format(x$CIAvgRad[1], scientific = TF, digits = dig),
                ",",
                format(x$CIAvgRad[2], scientific = TF, digits = dig),
                ")")

ciDif <- paste0("(", format(x$CIAvgDiffFom[1], scientific = TF, digits = dig),
                ",",
                format(x$CIAvgDiffFom[2], scientific = TF, digits = dig),
                ")")
...
colnames(df) <-c("FOM", "Analysis", "theta_0", "$CI_{\\theta_0}$", "theta_dot", "$CI_{\\theta_{\\bullet}}$", "psi_dot", "$CI_{\\psi_{\\bullet}}$",  "F", "ddf", "p")
# escape = FALSE is critical in getting Math right
df <- collapse_rows_df(collapse_rows_df(collapse_rows_df(collapse_rows_df(df,FOM),theta_0),theta_dot),psi_dot)

kbl(df, caption = captionStr, booktabs = TRUE, escape = FALSE) %>% collapse_rows(columns = c(3), valign = "middle") %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), font_size = 10) 
```
