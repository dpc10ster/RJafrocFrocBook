...
```{r optim-op-point-vary-lambda-froc, fig.cap="Scaled FROC plots for varying $\\mu$. Left panel: maximized wAFROC AUC was used to find optimal $\\zeta_1$. Right panel: maximized Youden-index was used to find optimal $\\zeta_1$. Dot colors: black is $\\nu = 0.6$, red is $\\nu = 0.7$, green is $\\nu = 0.8$ and blue is $\\nu = 0.9$.", fig.show='hold', echo=FALSE}
...
```{r optim-op-point-vary-nu-froc, fig.cap="Scaled FROC plots for varying $\\mu$. Left panel: maximized wAFROC AUC was used to find optimal $\\zeta_1$. Right panel: maximized Youden-index was used to find optimal $\\zeta_1$. Dot colors: black is $\\nu = 0.6$, red is $\\nu = 0.7$, green is $\\nu = 0.8$ and blue is $\\nu = 0.9$.", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]], plotArr[[2]], plotArr[[3]], plotArr[[4]], nrow = 2, ncol = 2)
```
...
plotArr <- array(list(), dim = 4)
for (i in 1:4) {
  x <- UtilPhysical2IntrinsicRSM(muArr[i], lambdaP = lambdaP, nuP = nuP)
  lambda <- x$lambda
  nu <- x$nu
  plotArr[[i]] <- PlotRsmOperatingCharacteristics(
    c(mu,mu),
