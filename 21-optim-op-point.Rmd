...
x1 <- cbind(x1, wAFROC = simplePrint(wAfrocArr), AUC = simplePrint(rocArr), Sp = simplePrint(1-fpfOptArr), Se = simplePrint(tpfOptArr))
...
kbl(x1, caption = "Summary of optimizations for combinations of 2-levels of $\\mu$, 2-levels of $\\lambda$ and 2-levels of $\\nu$. FOM = figure of merit, Sp = specificity and Se = sensitivity", booktabs = TRUE, escape = FALSE) %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c") 
```


* Mean wAFROC-AUC, (wAFROC, Youden) optimization = (`r simplePrint(mean(wAfrocArr[1,,,]))`, `r simplePrint(mean(wAfrocArr[2,,,]))`).
* Mean ROC-AUC, (wAFROC, Youden) optimization = (`r simplePrint(mean(rocArr[1,,,]))`, `r simplePrint(mean(rocArr[2,,,]))`).

* Mean (Sp, Se) wAFROC optimization = (`r simplePrint(1-mean(fpfOptArr[1,,,]))`, `r simplePrint(mean(tpfOptArr[1,,,]))`).
* Mean (Sp, Se) Youden optimization = (`r simplePrint(1-mean(fpfOptArr[2,,,]))`, `r simplePrint(mean(tpfOptArr[2,,,]))`).

* Mean (nlf, llf) wAFROC optimization = (`r simplePrint(mean(nlfOptArr[1,,,]))`, `r simplePrint(mean(llfOptArr[1,,,]))`).
* Mean (nlf, llf) Youden optimization = (`r simplePrint(mean(nlfOptArr[2,,,]))`, `r simplePrint(mean(llfOptArr[2,,,]))`).
...
```{r optim-op-point-vary-all-froc, fig.cap = "FROC plots with superimposed operating points for varying $\\nu$. The black dot corresponds to wAFROC AUC optimization and the red dot to Youden-index optimization.", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]], plotArr[[2]], plotArr[[3]], plotArr[[4]], nrow = 2, ncol = 2)
...
```{r optim-op-point-vary-all-wafroc, fig.cap="wAFROC plots for the two optimization methods. The color coding is as in previous figures.", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]], plotArr[[2]], plotArr[[3]], plotArr[[4]], nrow = 2, ncol = 2)
grid.arrange(plotArr[[5]], plotArr[[6]], plotArr[[7]], plotArr[[8]], nrow = 2, ncol = 2)
```
...
```{r optim-op-point-vary-all-roc, fig.cap="ROC plots for the two optimization methods. The color coding is as in previous figures.", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]], plotArr[[2]], plotArr[[3]], plotArr[[4]], nrow = 2, ncol = 2)
grid.arrange(plotArr[[5]], plotArr[[6]], plotArr[[7]], plotArr[[8]], nrow = 2, ncol = 2)
```
