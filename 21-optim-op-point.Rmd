...
The ROC, FROC and wAFROC curves are completely defined by the radiological search model (RSM) parameters, $\lambda$, $\nu$, $\mu$ and $\zeta_1$, which have the following meanings:

* The $\mu$ parameter, $\mu \ge 0$, is the perceptual signal to noise ratio of lesions. Higher values of $\mu$ lead to increased separation of two unit variance normal distributions determining the ratings of NLs and LL. As $\mu$ increases performance of the algorithm increases.

* The $\lambda$ parameter, $\lambda \ge 0$, determines the mean number of latent NLs per case. Lower values lead to fewer latent NL marks per case and increased algorithm performance. 

* The $\nu$ parameter, $0 \le \nu \le 1$, determines the probability of latent LLs. Higher values of $\nu$ lead to more latent LL marks.  

* The $\zeta_1$ parameter determines if a suspicious region found by the algorithm is actually marked (if the z-sample exceeds $\zeta_1$ the latent mark becomes an actual mark). Higher values correspond to more stringent reporting criteria. The higher the value of $\zeta_1$ the fewer the numbers of reported marks. 

* The objective is, given value of the other paramters, to optimize $\zeta_1$.  


In the TBA following sections each of the first three parameters is varied in turn and the corresponding optimal $\zeta_$ determined by maximizing one of two figures of merit (FOMs), namely, the wAFROC-AUC and the Youden-index. The value maximizing wAFROC-AUC is denoted $\zeta_{1;1\mu \lambda \nu}$ and the value maximizing the Youden-index is denoted $\zeta_{;2\mu \lambda \nu}$. 
...
colnames(x12) <- c("FOM", "$\\lambda$", "$\\zeta_1$", "wAFROC", "ROC", "$\\left( \\text{NLF}, \\text{LLF}\\right)$")
```



```{r optim-op-point-table1, echo=FALSE}
kbl(x12, caption = "Summary of optimization results for $\\mu = 2$, $\\nu = 0.9$ and 4 values of $\\lambda$. The wAFROC column always displays wAFROC-AUC, even though the optimized quantity may the Youden-index, as in the last four rows.", booktabs = TRUE, escape = FALSE) %>% collapse_rows(columns = c(1), valign = "middle") %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c") 
```


The ROC-AUCs are reported as a check of the less familar wAFROC-AUC figure of merit. It can be confirmed that the ordering of the two optimization methods is independent of whether it is measured via the wAFROC-AUC or the ROC-AUC: either way the wAFROC-AUC based optimizations yield higher AUC values than the Youden-index based optimizations.   
...
```{r optim-op-point-vary-lambda, fig.cap="Left panel: maximized wAFROC AUC was used to find optimal $\\zeta_1$. Right panel: maximized Youden-index was used to find optimal $\\zeta_1$. Dot colors: black is $\\lambda' = 1$, red is $\\lambda' = 2$, green is $\\lambda' = 5$ and blue is $\\lambda' = 10$.", fig.show='hold', echo=FALSE}
...
The right panel shows the optimal operating point when the Youden-index is maximized. It has the same general features as the previous example but, taken as a whole, the group of four operating points in the right panel are below-left those in the left panel, representing higher values of optimal $\zeta_1$, i.e., a more stringent reporting criteria. As seen in Table \@ref(tab:optim-op-point-table1) the overly strict criteria resulting from using Youden-index based optimizations, lead to lower true performance: i.e., lower wAFROC-AUC and lower ROC-AUC. It also yields lower values of LLF, i.e., fewer correctly localized lesions.
