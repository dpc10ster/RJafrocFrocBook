...
1. For either FOM as $\lambda$ increases the optimal threshold $\zeta_{1}$ increases and $\text{wAFROC}$, $\text{ROC}$ and $\text{LLF}$ decrease. Equivalently, CAD performance decreases, regardless of how it is measured (i.e., wAFROC-AUC or ROC-AUC).  
...
```{r, echo=FALSE}
x <- UtilRSM2Intrinsic(mu, lambda = lambda, nu = nu)
lambda <- x$lambda
nu <- x$nu
