...
```{r cache = FALSE, echo=FALSE}
wAfrocArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
zetaOptArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
rocArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
fpfOptArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
tpfOptArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
nlfOptArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
llfOptArr <- array(dim = c(2,length(muArr), length(nuPArr), length(lambdaPArr)))
for (i1 in 1:length(muArr)) {
  for (i2 in 1:length(nuPArr)) {
    for (i3 in 1:length(lambdaPArr)) {
      for (y in 1:2) {
        mu <- muArr[i1]
        nuP <- nuPArr[i2]
        lambdaP <- lambdaPArr[i3]
        if (y == 1) {
          x <- optimize(wAFROC, 
                        interval = c(-5,5), 
                        mu, 
...
```{r optim-op-point-table-vary-all-contents, echo=FALSE}
x1 <- cbind(FOM = rep("wAFROC",64), mu = muArr, lambda = lambdaPArr, nu = nuPArr, AUC = simplePrint(rocArr[1,,,]), Sp = simplePrint(1-fpfOptArr[1,,,]), Se = simplePrint(tpfOptArr[1,,,]))
x2 <- cbind(rep("Youden",64), mu = muArr, lambda = lambdaPArr, nu = nuPArr, AUC = simplePrint(rocArr[2,,,]), Sp = simplePrint(1-fpfOptArr[2,,,]), Se = simplePrint(tpfOptArr[2,,,]))
x12 <- as.data.frame(rbind(x1,x2))
colnames(x12) <- c("FOM", "$\\mu$", "$\\lambda$", "$\\nu$", "ROC-AUC", "Sp", "Se")
```



```{r optim-op-point-table-vary-all, echo=FALSE}
kbl(x12, caption = "Summary of optimization results for $\\mu = 2$, $\\nu = 0.9$ and 4 values of $\\lambda$. FOM = figure of merit, Sp = specificity and Se = sensitivity", booktabs = TRUE, escape = FALSE) %>% kable_styling(latex_options = c("basic", "scale_down", "HOLD_position"), row_label_position = "c") 
```


* Mean wAFROC-AUC, wAFROC optimization = `r simplePrint(mean(wAfrocArr[1,,,]))`.
* Mean wAFROC-AUC, Youden optimization = `r simplePrint(mean(wAfrocArr[2,,,]))`.

* Mean ROC-AUC, wAFROC optimization = `r simplePrint(mean(rocArr[1,,,]))`.
* Mean ROC-AUC, Youden optimization = `r simplePrint(mean(rocArr[2,,,]))`.

* Mean (Se, Sp) wAFROC optimization = (`r simplePrint(mean(fpfOptArr[1,,,]))`, `r simplePrint(1-mean(tpfOptArr[1,,,]))`).
* Mean (Se, Sp) Youden optimization = (`r simplePrint(mean(fpfOptArr[2,,,]))`, `r simplePrint(1-mean(tpfOptArr[2,,,]))`).
