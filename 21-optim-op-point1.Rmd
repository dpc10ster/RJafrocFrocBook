...
```{r vary-lambda-parameters2, echo=FALSE}
lesDistr <- c(0.5, 0.5)
...
```{r optim-op-point-vary-lambda-wafroc, fig.cap="wAFROC plots for the two optimization methods: the \"solid-green solid-red dashed-red\" curve corresponds to wAFROC-AUC optimization and the \"solid-green dashed-green\" curve corresponds to Youden-index optimization. The wAFROC-AUC optimizations yield greater performance than do Youden-index optimizations and the difference decreases with increasing $\\lambda$.", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]],plotArr[[2]],plotArr[[3]],plotArr[[4]], nrow = 2, ncol = 2)
```
...
```{r optim-op-point-vary-lambda-roc, fig.cap="ROC plots for the two optimization methods: the \"solid-green solid-red dashed-red\" curve corresponds to wAFROC-AUC optimization and the \"solid-green dashed-green\" curve corresponds to Youden-index optimization. The wAFROC-AUC optimizations yield greater performance than Youden-index optimizations and the difference decreases with increasing $\\lambda$.", fig.show='hold', echo=FALSE}
grid.arrange(plotArr[[1]],plotArr[[2]],plotArr[[3]],plotArr[[4]], nrow = 2, ncol = 2)
```
...
```{r binormal-model-threshold-dependence-2, fig.cap="In the region above the dot the proper curve is above the dotted line, meaning performance of an observer who adopts a finite $\\zeta_1$ is less than performance of an observer who adopts $\\zeta_1 = -\\infty$.", fig.show='hold', out.width="300pt", fig.align='center', echo=FALSE}
grid.arrange(p1)
```
...
```{r optim-op-point-application-froc, fig.cap="FROC plots with superposed optimal operating points. The red dot is using wAFROC-AUC optimization and black dot is using Youden-index optimization.", fig.show='hold', echo=FALSE}
grid.arrange(froc)
```
