...
* The intrinsic (i.e., $\mu$ independent) $\lambda$ parameter determines the number of non-lesion localizations, NLs, per case (location level "false positives"). Lower values lead to fewer NL marks per case and increased algorithm performance. It is related to the physical $\lambda'$ parameter by $\lambda' = \lambda/\mu$. The physical parameter $\lambda'$ equals the mean of the assumed Poisson distribution of NLs per case.

* The intrinsic $\nu$ parameter determines the probability of lesion localizations, LLs, (location level "true positives"). Higher values lead to more LL marks. It is related to the physical $\nu'$ parameter by $\nu' = 1 - \exp(-\mu \nu)$. The physical parameter $\nu'$ equals the success probability of the assumed binomial distribution of LLs per case.
...
The wAFROC figure of merit is calculated by the `RJafroc` functions `UtilAnalyticalAucsRSM`. The Youden-index is defined as the sum of sensitivity and specificity minus 1. Sensitivity is computed by `RSM_yROC` and specificity by `(1 - RSM_xROC)`. The RSM arguments, some of which are passed to the functions, are $\mu$, $\lambda'$, $\nu'$, `lesDistr` and `relWeights.` The last two specify the number of lesions per case and their weights (clinical importance). The code in this chapter assumes half of the diseased cases contain one lesion and the rest contain two lesions and the lesions are equally weighted.



```{r functions_2be_maximized, echo=FALSE, attr.source = ".numberLines"}

wAFROC <- function (
  zeta1, mu, lambdaP1, nuP1, 
  lesDistr, 
  relWeights) {
...
For $\mu = 2$ and $\nu = 1$, wAFROC-AUC and Youden-index based optimizations were performed for $\lambda' = 1, 2, 5, 10$. The following quantities were calculated:

* `zetaOptArr`, a [2,4] array, the optimal thresholds $\zeta_1$; the first index corresponds to wAFROC-based or Youden-index based optimizations. The second index corresponds to the 4 values of $\lambda$.
* `wAfrocArr`, a [2,4] array, the wAFROC-AUC values using either wAFROC-based or Youden-index based optimizations. For consistency we always report wAFROC-AUC even when the optimized quantity is the Youden-index;
* `rocArr`, a [2,4] array, the AUCs under the ROC curves corresponding to wAFROC-based or Youden-index based optimizations;   
* `nlfOptArr`, a [2,4] array, the abscissae of the optimal reporting points on the FROC curve corresponding to wAFROC-based or Youden-index based optimizations;   
* `llfOptArr`, a [2,4] array, the ordinates of the optimal reporting points on the FROC curve corresponding to wAFROC-based or Youden-index based optimizations.   
...
mu <- 2
nuP1 <- 1
lambdaP1Arr <- c(1, 2, 5,10)
wAfrocArr <- array(dim = c(2,length(lambdaP1Arr)))
zetaOptArr <- array(dim = c(2,length(lambdaP1Arr)))
rocArr <- array(dim = c(2,length(lambdaP1Arr)))
nlfOptArr <- array(dim = c(2,length(lambdaP1Arr)))
llfOptArr <- array(dim = c(2,length(lambdaP1Arr)))
lesDistr <- c(0.5, 0.5)
relWeights <- c(0.5, 0.5)
for (y in 1:2) {
  for (i in 1:length(lambdaP1Arr)) {
    lambdaP1 <- lambdaP1Arr[i]
    if (y == 1) {
      x <- optimize(wAFROC, 
                    interval = c(-5,5), 
                    mu, 
...
x1 <- cbind(rep("wAFROC",4), lambda = lambdaP1Arr, zeta1 = simplePrint(zetaOptArr[1,]), wAFROC = simplePrint(wAfrocArr[1,]), ROC = simplePrint(rocArr[1,]), OptOpPt = OpPtStr(nlfOptArr[1,], llfOptArr[1,]))
x2 <- cbind(rep("Youden",4), lambda = lambdaP1Arr, zeta1 = simplePrint(zetaOptArr[2,]), wAFROC = simplePrint(wAfrocArr[2,]), ROC = simplePrint(rocArr[2,]), OptOpPt = OpPtStr(nlfOptArr[2,], llfOptArr[2,]))
...
* The advantage of wAFROC-AUC over Youden-index based optimizations, as measured by the differences between the corresponding wAFROC-AUCs, decreases with increasing $\lambda$: `wAfrocArr[1,] - wAfrocArr[2,]` = `r simplePrint(wAfrocArr[1,] - wAfrocArr[2,])`, where the successive values correspond to $\lambda = 1, 5, 10, 15$.   

The $\lambda'$ Poisson parameter controls the average number of perceived NLs per case. For example, for $\mu = 2$ and $\lambda = 1$, the average number is $\lambda' = \lambda /\mu = 0.5$, i.e., an average of one perceived NL every two non-diseased case. With increasing numbers of NLs per case it is necessary to increase the reporting threshold (i.e., adopt a stricter reporting criteria) and LLF consequently decreases. Also, overall CAD/AI performance, regardless of how it is measured (i.e., wAFROC-AUC or ROC-AUC), decreases.   
...
for (y in 1:2) {
  ...
  lambdaP1 <- lambdaP1Arr[i] / mu
  # scale x-axis to range (0,1) by dividing by lambdaP1
  xFROC <- RSM_xFROC(z, lambdaP1) / lambdaP1
  yFROC <- RSM_yFROC(z, mu, nuP1)
  ...
  for ( i in 1:length(lambdaP1Arr)) {
    ...
    # by lambdaP1 to match scaling of x-axis
    optPt <- data.frame(
      NLF_scaled = nlfOptArr[y,i] / lambdaP1, 
      LLF = llfOptArr[y,i])
...
```{r optim-op-point-vary-lambda, fig.cap="Left panel: maximized wAFROC AUC was used to find optimal $\\zeta_1$. Right panel: maximized Youden-index was used to find optimal $\\zeta_1$. Dot colors: black is $\\lambda = 1$, red is $\\lambda = 5$, green is $\\lambda = 10$ and blue is $\\lambda = 15$.", fig.show='hold', echo=FALSE}
...
x1 <- cbind(rep("wAFROC",4), nu = nuArr, zeta1 = simplePrint(zetaOptArr[1,]), wAFROC = simplePrint(wAfrocArr[1,]), ROC = simplePrint(rocArr[1,]), OptOpPt = OpPtStr(nlfOptArr[1,], llfOptArr[1,]))
x2 <- cbind(rep("Youden",4), nu = nuArr, zeta1 = simplePrint(zetaOptArr[2,]), wAFROC = simplePrint(wAfrocArr[2,]), ROC = simplePrint(rocArr[2,]), OptOpPt = OpPtStr(nlfOptArr[2,], llfOptArr[2,]))
x12 <- as.data.frame(rbind(x1,x2))
...
* The optimal threshold $\zeta_1$ decreases resulting in more marks; wAFROC-AUC increases; ROC-AUC increases and the optimal operating point on the FROC moves to higher LLF values, i.e., higher values of lesion-level "sensitivity".
...
TBA The exception is for $\nu = 0.1$ for which the Youden $\zeta_1$ is smaller than that for wAFROC..... increases the optimal threshold increases, but more slowly. The increasing separation of the two underlying probability density functions that generate the ROC causes the optimal threshold to increase (similar to the explanation in Section \@ref(optim-op-point-vary-mu)).
...
for (y in 1:2) {
  for (i in 1:length(muArr)) {
    x <- UtilIntrinsic2PhysicalRSM(muArr[i], lambda, nu)
    lambdaP1 <- x$lambdaP1
    nuP1 <- x$nuP1
    if (y == 1) {
      x <- optimize(wAFROC, 
...
for (y in 1:2) {
  froc[[y]] <- ggplot2::ggplot()
  for ( i in 1:length(muArr)) {
    x <- UtilIntrinsic2PhysicalRSM(muArr[i], lambda = lambda, nu)
    lambdaP1 <- x$lambdaP1
    nuP1 <- x$nuP1
    z <- seq(-5,zetaOptArr[y,i]+5,0.1)
    xFROC <- RSM_xFROC(z, lambdaP1)
    yFROC <- RSM_yFROC(z, muArr[i], nuP1) 
    df_froc <- data.frame(NLF = xFROC, LLF = yFROC)
    froc[[y]] <- froc[[y]] + 
      geom_line(data = df_froc, aes(x = NLF, y = LLF))
    optPt <- data.frame(
...
mu <- fit$mu
lambdaP1 <- fit$lambdaP1
nuP1 <- fit$nuP1
x <- UtilPhysical2IntrinsicRSM(mu, lambdaP1, nuP1)
lambda <- x$lambda
nu <- x$nu

wAfrocArr <- array(dim = c(2))
zetaOptArr <- array(dim = c(2))
rocArr <- array(dim = c(2))
...
for (y in 1:2) {
  if (y == 1) {
    ...
    rocArr[y] <- UtilAnalyticalAucsRSM(
      mu, 
      lambdaP = lambdaP1, 
      nuP = nuP1, 
      zeta1 = x$minimum, 
...
x1 <- cbind("wAFROC", lambda = simplePrint(lambda), zeta1 = simplePrint(zetaOptArr[1]), wAFROC = simplePrint(wAfrocArr[1]), ROC = simplePrint(rocArr[1]), OptOpPt = OpPtStr(nlfOptArr[1], llfOptArr[1]))
x2 <- cbind("Youden", lambda = simplePrint(lambda), zeta1 = simplePrint(zetaOptArr[2]), wAFROC = simplePrint(wAfrocArr[2]), ROC = simplePrint(rocArr[2]), OptOpPt = OpPtStr(nlfOptArr[2], llfOptArr[2]))
x12 <- as.data.frame(rbind(x1,x2))
colnames(x12) <- c("FOM", "lambda", "zeta1", "wAFROC", "ROC", "OptOpPt")
```
