...
## TBA How much finished 90% {#rsm-other-predictions-how-much-finished}
...
One calculates $\text{LLF}$ as follows: 
...
```{r rsm-other-predictions-froc-plots-lambdai-nui, fig.cap="RSM-predicted FROC curves using intrinsic parameters $\\lambda_i = 2$ and $\\nu_i = 0.5$. Top left: $\\mu = 0.1$; Top right: $\\mu = 1$; Bottom left: $\\mu = 2$; Bottom right: $\\mu = 4$. Each plot is **not** contained within the unit square which makes it impossible to define the FROC-AUC as a figure of merit.", fig.show='hold', echo=TRUE}
...
> 
* In the top-left plot, because $\lambda = 20$ defines the end-point abscissa and $\nu = 4.9 \times 10^{-2}$ defines the end-point ordinate, the FROC curve hugs the x-axis approaching $(20, 4.9 \times 10^{-2})$. This is the chance line FROC for very small $\mu$. 
...
This equation is implemented in `UtilAnalyticalAucsRSM`. Help is available [here](https://dpc10ster.github.io/RJafroc/reference/UtilAnalyticalAucsRSM.html). A skeleton code is shown below:
...
```{r rsm-other-predictions-wafroc-plots-lambda-nu, fig.cap="RSM-predicted wAFROC curves using $\\lambda, \\nu$ paramterization. Top left: $\\mu = 0.1$, $\\lambda = 1$ and $\\nu = 0.2$. Top right: $\\mu = 1$, $\\lambda = 2$ and $\\nu = 0.5$. Bottom left: $\\mu = 2$, $\\lambda = 3$ and $\\nu = 0.7$. Bottom right: $\\mu = 4$, $\\lambda = 4$ and $\\nu = 0.9$. As $\\mu$ increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit. Each wAFROC curve is slightly above the corresponding AFROC curve.", fig.show='hold', echo=FALSE}
grid.arrange(p_wAFROC_lambda_nu[[1]],p_wAFROC_lambda_nu[[2]],p_wAFROC_lambda_nu[[3]],p_wAFROC_lambda_nu[[4]], ncol = 2)
```



### ROC curves are generally above the AFROC curves {#rsm-other-predictions-roc-above-afroc}


Using previous equations for the ROC-TPF and the AFROC-LLF, and focusing on cases with $L$ lesions, one has:
...
* $\text{exp}\left ( - \lambda \Phi \left ( - \zeta \right )\right )$ is non-negative and less than one. 
...
### wAFROC. vs AFROC curves? {#rsm-other-predictions-wafroc-above-afroc}

An expression can be derived as follows for the difference between wLLF and LLF:


\begin{equation} 
\left.
\begin{aligned}
\text{wLLF}-\text{LLF} =& \\ \Phi\left ( \mu - \zeta \right )\sum_{L=1}^{L_{max}} f_L  \sum_{l_2=1}^{L} \left( \text{W}_{Ll_2} -\frac{1}{L} \right) \, l_2 \,\,  \text{pmf}_{B}\left ( l_2, L, \nu \right )
\end{aligned}
\right \}
...
```{r rsm-other-predictions-wafroc-plots-lambda-nu-code_Lo_Hi, echo=FALSE}
relWeightsLo =  c(0.7, 0.1, 0.1, 0.1)
relWeightsHi =  c(0.1, 0.1, 0.1, 0.7)
lesDistrLo <- c(0.7, 0.1, 0.1, 0.1)
lesDistrHi <- c(0.1, 0.1, 0.1, 0.7)

p_wAFROC_lambda_nu_Lo <- array(list(), dim = c(length(lambdaArr)))
p_wAFROC_lambda_nu_Hi <- array(list(), dim = c(length(lambdaArr)))
for (i in 1:length(lambdaArr)) {
  mu <- muArr[i]
  lambda <- lambdaArr[i]
  nu <- nuArr[i]
  ret1 <- PlotRsmOperatingCharacteristics(
    mu, lambda, nu,
    lesDistr = lesDistrLo,
    relWeights = relWeightsLo,
    OpChType = "wAFROC",
    legendPosition  = "none"
  )
}
p_wAFROC_lambda_nu_Lo[[i]] <- ret1$wAFROCPlot 
...
for (i in 1:length(lambdaArr)) {
  ...
  ret1 <- PlotRsmOperatingCharacteristics(
    mu, lambda, nu,
    lesDistr = lesDistrHi,
    relWeights = relWeightsHi,
    OpChType = "wAFROC",
    legendPosition  = "none"
  )
  p_wAFROC_lambda_nu_Hi[[i]] <- ret1$wAFROCPlot 
}
```
...
```{r echo=T}
for (i in 1:4) {
  comparison <- all(p_wAFROC_lambda_nu_Lo[[i]]$data$wLLF 
                    <= p_AFROC_lambda_nu[[i]]$data$LLF)
  cat(sprintf("mu = %6.3f, comparison = %5s", muArr[i], comparison), "\n")
}
```


TBA The `FALSE` value corresponds to very small $\mu = 0.1$. The remaining $\mu$ values yield `TRUE`.


```{r echo=T}
for (i in 1:4) {
  comparison <- all(p_wAFROC_lambda_nu_Hi[[i]]$data$wLLF 
                    >= p_AFROC_lambda_nu[[i]]$data$LLF)
  cat(sprintf("mu = %6.3f, comparison = %5s", muArr[i], comparison), "\n")
}
```
