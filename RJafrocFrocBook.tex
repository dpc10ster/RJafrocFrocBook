...
\hypertarget{computing-aucs}{%
\chapter{Computing FROC paradigm AUCs}\label{computing-aucs}}

\hypertarget{computing-aucs-how-much-finished}{%
\section{TBA How much finished}\label{computing-aucs-how-much-finished}}

50\%

\hypertarget{computing-aucs-intro}{%
\section{TBA Introduction}\label{computing-aucs-intro}}

Chapter \texttt{\textbackslash{}@ref(empirical)} introduced several empirical operating characteristic plots from FROC data. Expressions were given for computing \emph{operating points} for each such plot from z-samples. The areas or AUCs under these plots were also defined.

Calculating areas from operating points using planimetry or geometry is tedious at best. This chapter defines formulae for calculating the AUCs directly from the z-samples or ratings. These expressions are analogous to computing the Wilcoxon statistic for ROC ratings data since it is known \citep{RN2174} that this statistic equals the AUC under the empirical ROC plot.
...
\hypertarget{computing-aucs-afroc}{%
\section{Computing empirical AFROC AUC}\label{computing-aucs-afroc}}

\(\text{A}_{AFROC}\) is the area under the empirical AFROC. It can be computed from the z-samples as follows.

The highest rating \(\text{FP}_{k_1 1}\) on non-diseased case \(k_1 1\) was defined in Section \eqref{eq:empirical-FP}. The function \(\psi\left ( x, y \right )\) was defined (see chapter TBA on empirical AUC in \texttt{RJafrocRocBook}) by:
...
\end {aligned}
\label{eq:computing-aucs-theta-afroc}
\end{equation}

\hypertarget{range-of-afroc-auc-statistic}{%
\subsection{Range of AFROC AUC statistic}\label{range-of-afroc-auc-statistic}}
...
\hypertarget{computing-aucs-wafroc}{%
\section{Computing empirical wAFROC AUC}\label{computing-aucs-wafroc}}

The empirical weighted-AFROC plot and lesion weights were defined in Section TBA \texttt{\textbackslash{}@ref(empirical-wAFROC)}. The corresponding AUC can be computed \citep{RN1385} by including the lesion weights \(W_{k_2 l_2}\) inside the summations (but outside the kernel function):
...
\end {aligned}
\label{eq:computing-aucs-theta-wafroc}
\end{equation}
...
\label{eq:computing-aucs-theta-constraint-weights}
\end{equation}

\hypertarget{computing-aucs-Discussion}{%
\section{Discussion}\label{computing-aucs-Discussion}}
...
\hypertarget{computing-aucs-references}{%
\section{References}\label{computing-aucs-references}}
