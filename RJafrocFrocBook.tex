...
\hypertarget{froc-curve-slopes-near-origin-and-end-point}{%
\subsection{FROC curve slopes near origin and end-point}\label{froc-curve-slopes-near-origin-and-end-point}}
...
\hypertarget{froc-plots-using-standard-lambda-nu-parameters}{%
\subsection{\texorpdfstring{FROC plots using standard \(\lambda, \nu\) parameters}{FROC plots using standard \textbackslash lambda, \textbackslash nu parameters}}\label{froc-plots-using-standard-lambda-nu-parameters}}
...
\hypertarget{afroc-curve-slopes-near-origin-and-end-point}{%
\subsection{AFROC curve slopes near origin and end-point}\label{afroc-curve-slopes-near-origin-and-end-point}}
...
\hypertarget{afroc-plots-using-standard-lambda-nu-parameters}{%
\subsection{\texorpdfstring{AFROC plots using standard \(\lambda, \nu\) parameters}{AFROC plots using standard \textbackslash lambda, \textbackslash nu parameters}}\label{afroc-plots-using-standard-lambda-nu-parameters}}

\begin{figure}
\centering
\includegraphics{08-rsm-predictions2_files/figure-latex/rsm-pred2-afroc-plots-lambda-nu-1.pdf}
\caption{\label{fig:rsm-pred2-afroc-plots-lambda-nu}RSM-predicted AFROC curves using \(\lambda, \nu\) paramterization. Top left: \(\mu = 0.0001\), \(\lambda = 1\) and \(\nu = 0.2\). Top right: \(\mu = 1\), \(\lambda = 2\) and \(\nu = 0.5\). Bottom left: \(\mu = 2\), \(\lambda = 3\) and \(\nu = 0.7\). Bottom right: \(\mu = 4\), \(\lambda = 4\) and \(\nu = 0.9\). As \(\mu\) increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit.}
\end{figure}
...
\item
  From Eqn. \eqref{eq:rsm-pred2-froc-end-point} and \eqref{eq:rsm-pred-tpf} it follows that \(\text{TPF}_{\text{max}} \geq \text{LLF}_{max}\) with the equality holding in the limit \(\mu \rightarrow \infty\).
\end{enumerate}

The reason \(\text{TPF}_{\text{max}} \geq \text{LLF}_{max}\) is because the ROC gives credit for incorrect localizations on diseased cases, while the AFROC does not. This is the well-known ``right for wrong reason'' argument \citep{bunch1977free}, originally advanced in 1977, against usage of the ROC for localization tasks.
...
\hypertarget{wafroc-plots-using-standard-lambda-nu-parameters}{%
\subsection{\texorpdfstring{wAFROC plots using standard \(\lambda, \nu\) parameters}{wAFROC plots using standard \textbackslash lambda, \textbackslash nu parameters}}\label{wafroc-plots-using-standard-lambda-nu-parameters}}
