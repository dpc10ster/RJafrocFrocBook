...
To extend the results to varying numbers of lesions per diseased case, one averages the right hand side of \eqref{eq:rsm-pred-tpf} over the fraction of diseased cases with \(L\) lesions:
...
{\centering \includegraphics[width=300pt]{images/rsm-pred/util-analytical-aucs-rsm} 
...
{\centering \includegraphics[width=300pt]{images/rsm-pred/util-aucs-binormal} 
...
{\centering \includegraphics[width=300pt]{images/rsm-pred/PlotRsmOperatingCharacteristics} 
...
\hypertarget{rsm-pred2}{%
\chapter{RSM FROC/AFROC/wAFROC predictions}\label{rsm-pred2}}

\hypertarget{rsm-pred2-how-much-finished}{%
\section{TBA How much finished}\label{rsm-pred2-how-much-finished}}

70\%
Need to move lambda, nu parameterization to appendix; too distracting
physical vs.~intrinsic; edit chapter 4

\hypertarget{rsm-pred2-intro}{%
\section{TBA Introduction}\label{rsm-pred2-intro}}
...
\hypertarget{rsm-pred2-froc-curve}{%
\section{RSM-predicted FROC curve}\label{rsm-pred2-froc-curve}}

\hypertarget{formulae}{%
\subsection{Formulae}\label{formulae}}

From a property of the Poisson distribution, namely its mean equals the \(\lambda\) parameter of the distribution, it follows that the expected number of \emph{latent} NLs per case is \(\lambda\). One multiplies this by the probability that a z-sample from \(N(0,1)\) exceeds \(\zeta\), i.e.~by \(\Phi(-\zeta)\), to obtain the expected number of \emph{marked} NLs per case, i.e., NLF:

\begin{equation}
\text{NLF} \left ( \zeta, \lambda \right ) = \lambda \Phi \left (-\zeta \right )
\label{eq:rsm-pred2-nlf}
\end{equation}
...
\end{aligned}
\right \}
\label{eq:rsm-pred2-llf}
\end{equation}

Summarizing, the coordinates of the RSM-predicted operating point on the FROC curve for threshold \(\zeta\) are given by Eqn. \eqref{eq:rsm-pred2-nlf} and Eqn. \eqref{eq:rsm-pred2-llf}. The FROC curve starts at (0,0) and ends at \(\left ( \lambda, \nu \right )\). The x-coordinate does not extend to arbitrarily large values and the y-coordinate does not approach unity (unless \(\nu = 1\)). The constrained end-point property, demonstrated previously for the ROC curve, also applies to the FROC curve:
...
\end{aligned}
\right \}
\label{eq:rsm-pred2-froc-end-point}
\end{equation}

\hypertarget{froc-plots-using-intrinsic-lambda_i-nu_i-parameters}{%
\subsection{\texorpdfstring{FROC plots using intrinsic \(\lambda_i, \nu_i\) parameters}{FROC plots using intrinsic \textbackslash lambda\_i, \textbackslash nu\_i parameters}}\label{froc-plots-using-intrinsic-lambda_i-nu_i-parameters}}

The following code shows how to generate FROC plots using the intrinsic \(\lambda_i = 2\) and \(\nu_i = 0.5\) parameters for 4 values of \(\mu\) contained in the array \texttt{muArr\ \textless{}-\ c(0.0001,1,2,4)} (for technical reasons the value 0 is not allowed). A list array \texttt{plotArr\_FROC\_lambdai\_nui} is created to contain the four plots. The intrinsic intrinsic \(\lambda_i, \nu_i\) parameters are converted to \(\lambda, \nu\) using the function \texttt{UtilIntrinsic2RSM()} and the values are printed out using the \texttt{cat()} function. The plots are generated using \texttt{PlotRsmOperatingCharacteristics()}: this function accepts the parameters \(\mu, \lambda, \nu\) and \texttt{opChType\ =\ "FROC"}. Online help on this function is available \href{https://dpc10ster.github.io/RJafroc/reference/PlotRsmOperatingCharacteristics.html}{here}. The code \texttt{plotArr\_FROC\_lambdai\_nui{[}{[}i{]}{]}\ \textless{}-\ ret1\$FROCPlot} saves the plot to the previously created list-array.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{muArr <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.0001}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{4}\NormalTok{)}
\NormalTok{lambda_i <-}\StringTok{ }\DecValTok{2}
\NormalTok{nu_i <-}\StringTok{ }\FloatTok{0.5}
\NormalTok{plotArr_FROC_lambdai_nui <-}\StringTok{ }\KeywordTok{array}\NormalTok{(}\KeywordTok{list}\NormalTok{(), }\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(}\KeywordTok{length}\NormalTok{(muArr)))}
...
\NormalTok{  nu <-}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{nu}
  \KeywordTok{cat}\NormalTok{(}\StringTok{"lambda = "}\NormalTok{, lambda, }\StringTok{", nu = "}\NormalTok{, nu, }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
...
\NormalTok{  )}
\NormalTok{  plotArr_FROC_lambdai_nui[[i]] <-}\StringTok{ }\NormalTok{ret1}\OperatorTok{$}\NormalTok{FROCPlot }
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{suppressWarnings}\NormalTok{(}\KeywordTok{grid.arrange}\NormalTok{(plotArr_FROC_lambdai_nui[[}\DecValTok{1}\NormalTok{]],}
\NormalTok{plotArr_FROC_lambdai_nui[[}\DecValTok{2}\NormalTok{]],}
\NormalTok{plotArr_FROC_lambdai_nui[[}\DecValTok{3}\NormalTok{]],}
\NormalTok{plotArr_FROC_lambdai_nui[[}\DecValTok{4}\NormalTok{]], }\DataTypeTok{ncol =} \DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\includegraphics{08-rsm-predictions2_files/figure-latex/rsm-pred2-froc-plots-lambdai-nui-1.pdf}
\caption{\label{fig:rsm-pred2-froc-plots-lambdai-nui}RSM-predicted FROC curves using intrinsic parameters \(\lambda_i = 2\) and \(\nu_i = 0.5\). Top left: \(\mu = 0.0001\); Top right: \(\mu = 1\); Bottom left: \(\mu = 2\); Bottom right: \(\mu = 4\). Each plot is \textbf{not} contained within the unit square which makes it impossible to define the AUC as a figure of merit.}
\end{figure}
...
\hypertarget{rsm-pred2-froc-afroc-curve}{%
\section{RSM-predicted AFROC curve}\label{rsm-pred2-froc-afroc-curve}}

\hypertarget{formulae-1}{%
\subsection{Formulae}\label{formulae-1}}

The AFROC x-coordinate is the same as the ROC x-coordinate and Eqn. \eqref{eq:rsm-pred-fpf} applies. The AFROC y-coordinate is identical to the FROC y-coordinate and Eqn. \eqref{eq:rsm-pred2-llf} applies. Note that the expression is independent of the number of lesions in the dataset or their distribution (unlike the AFROC the weighted AFROC does depend on the lesion distribution).
...
\end{aligned}
\right \}
\label{eq:rsm-pred2-afroc-end-point}
\end{equation}

It too has the constrained end-point property.

\hypertarget{afroc-plots-using-intrinsic-lambda_i-nu_i-parameters}{%
\subsection{\texorpdfstring{AFROC plots using intrinsic \(\lambda_i, \nu_i\) parameters}{AFROC plots using intrinsic \textbackslash lambda\_i, \textbackslash nu\_i parameters}}\label{afroc-plots-using-intrinsic-lambda_i-nu_i-parameters}}
...
\begin{figure}
\centering
\includegraphics{08-rsm-predictions2_files/figure-latex/rsm-pred2-afroc-plots-lambdai-nui-1.pdf}
\caption{\label{fig:rsm-pred2-afroc-plots-lambdai-nui}RSM-predicted AFROC curves using intrinsic parameters \(\lambda_i = 2\) and \(\nu_i = 0.5\). Top left: \(\mu = 0.0001\); Top right: \(\mu = 1\); Bottom left: \(\mu = 2\); Bottom right: \(\mu = 4\). TBA TBA. As \(\mu\) increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit.}
\end{figure}
...
\hypertarget{rsm-pred2-no-marks}{%
\subsection{Case of the reader who does not make any marks}\label{rsm-pred2-no-marks}}
...
\hypertarget{rsm-pred2-wafroc-curve}{%
\section{RSM-predicted wAFROC curve}\label{rsm-pred2-wafroc-curve}}

\hypertarget{formulae-2}{%
\subsection{Formulae}\label{formulae-2}}

The wAFROC abscissa is identical to the ROC abscissa, i.e., Eqn. \eqref{eq:rsm-pred-fpf} applies. The wAFROC ordinate is calculated using:
...
\label{eq:rsm-pred2-wllf}
\end{equation}

\begin{itemize}
\item
  The Binomial probability mass function \(\text{pmf}_{Bin}\) was defined in Eqn. \eqref{eq:rsm-binomial-pmf}.
\item
...
\item
  \(\mathbf{W}\) is the (lower triangular) square matrix with \(L_{max}\) rows and columns containing the weights, where each row sums to unity, see example below (the unused matrix elements are set to \(-\infty\)). The relative lesion weights are denoted in the code \texttt{relWeights}. For example, \texttt{relWeights\ =\ \ c(0.2,\ 0.3,\ 0.1,\ 0.5)} whose meaning is as follows:
...
\item
  Eqn. \eqref{eq:rsm-pred2-wllf} is implemented in \texttt{UtilAnalyticalAucsRSM}.
\item
...
\begin{Highlighting}[]
\NormalTok{lesDistr <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.6}\NormalTok{, }\FloatTok{0.2}\NormalTok{, }\FloatTok{0.1}\NormalTok{, }\FloatTok{0.1}\NormalTok{)}
\KeywordTok{UtilLesionWeightsMatrixLesDistr}\NormalTok{(lesDistr, }\DataTypeTok{relWeights =} \DecValTok{0}\NormalTok{)[,}\OperatorTok{-}\DecValTok{1}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{wafroc-plots-intrinsic-lambda_i-nu_i-parameters}{%
\subsection{\texorpdfstring{wAFROC plots intrinsic \(\lambda_i, \nu_i\) parameters}{wAFROC plots intrinsic \textbackslash lambda\_i, \textbackslash nu\_i parameters}}\label{wafroc-plots-intrinsic-lambda_i-nu_i-parameters}}

\hypertarget{formulae-3}{%
\subsection{Formulae}\label{formulae-3}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plotArr_wAFROC_lambdai_nui <-}\StringTok{ }\KeywordTok{array}\NormalTok{(}\KeywordTok{list}\NormalTok{(), }\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(}\KeywordTok{length}\NormalTok{(muArr)))}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(muArr)) \{}
\NormalTok{  mu <-}\StringTok{ }\NormalTok{muArr[i]}
\NormalTok{  ret <-}\StringTok{ }\KeywordTok{UtilIntrinsic2RSM}\NormalTok{(mu, }\DataTypeTok{lambda_i =}\NormalTok{ lambda_i, }\DataTypeTok{nu_i =}\NormalTok{ nu_i)}
\NormalTok{  lambda <-}\StringTok{ }\NormalTok{ret}\OperatorTok{$}\NormalTok{lambda  }
...
\NormalTok{    mu, lambda, nu,}
    ...
    \DataTypeTok{legendPosition  =} \StringTok{"none"}
\NormalTok{  )}
\NormalTok{  plotArr_wAFROC_lambdai_nui[[i]] <-}\StringTok{ }\NormalTok{ret1}\OperatorTok{$}\NormalTok{wAFROCPlot }
  \CommentTok{#+ ggtitle(paste0("mu = ", as.character(muArr[i]), }
...
\begin{figure}
\centering
\includegraphics{08-rsm-predictions2_files/figure-latex/rsm-pred2-wafroc-plots-lambdai-nui-1.pdf}
\caption{\label{fig:rsm-pred2-wafroc-plots-lambdai-nui}RSM-predicted wAFROC curves using intrinsic parameters \(\lambda_i = 2\) and \(\nu_i = 0.5\). Top left: \(\mu = 0.0001\). Top right: \(\mu = 1\). Bottom left: \(\mu = 2\). Bottom right: \(\mu = 4\). As \(\mu\) increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square making it easy to define the AUC as a figure of merit. Each wAFROC curve is slightly above the corresponding AFROC curve.}
\end{figure}

\hypertarget{rsm-pred2-discussion-summary}{%
\section{Discussion / Summary}\label{rsm-pred2-discussion-summary}}
...
\hypertarget{slope-of-the-froc-curve}{%
\subsection{Slope of the FROC curve}\label{slope-of-the-froc-curve}}
...
\emph{The last prediction is physically unreasonable.} For zero contrast lesions the observer should not be able to localize any lesions at finite NLF. The unreasonable prediction is occurring because one is feeding the RSM unrealistic parameters. As stated earlier, for zero \(\mu\) one expects \(\lambda = \infty\) and \(\nu = 0\), not \(\lambda = 1\) and \(\nu = 0.2\), as in the top-left panel plot below.

In terms of intrinsic parameters, Eqn. \eqref{eq:rsm-pred2-froc-slope} reduces to:
...
\begin{figure}
\centering
\includegraphics{08-rsm-predictions2_files/figure-latex/rsm-pred2-froc-plots-lambda-nu-1.pdf}
\caption{\label{fig:rsm-pred2-froc-plots-lambda-nu}RSM-predicted FROC curves using \(\lambda, \nu\) paramterization. Top left: \(\mu = 0.0001\), \(\lambda = 1\) and \(\nu = 0.2\). Top right: \(\mu = 1\), \(\lambda = 2\) and \(\nu = 0.5\). Bottom left: \(\mu = 2\), \(\lambda = 3\) and \(\nu = 0.7\). Bottom right: \(\mu = 4\), \(\lambda = 4\) and \(\nu = 0.9\). The top-left panel is an unrealistic prediction because of the supplied unrealistic parameters \(\lambda =1, \nu = 0.2\) for small \(\mu\).}
\end{figure}

\hypertarget{slope-of-the-afroc-curve}{%
\subsection{Slope of the AFROC curve}\label{slope-of-the-afroc-curve}}
...
\end{aligned}
\right \} 
\label{eq:rsm-pred2-afroc-slope}
\end{equation}
...
\end{aligned}
\right \} 
\label{eq:rsm-pred2-afroc-slope3}
\end{equation}

Where \(\text{slope}_{\text{FROC}}\) is the slope of the FROC, Eqn. \eqref{eq:rsm-pred2-froc-slope}. Since \(0 \le \text{FPF} \le \text{FPF}_{\text{max}}\) and \(\text{FPF}\) increases as \(\zeta\) decreases, the slope of the AFROC equals that of the FROC at the origin and subsequently increases over that of the FROC as the end-point is approached.
...
\def\labelenumi{\arabic{enumi}.}
\item
  The AFROC plot is independent of the number of lesions per case. This is not true for the wAFROC, as will shortly become clear, and for the ROC: the ROC ordinate increases with increasing numbers of lesions per case.
\item
  From Eqn. \eqref{eq:rsm-pred2-froc-end-point} and \eqref{eq:rsm-pred-tpf} it follows that \(\text{TPF}_{\text{max}} \geq \text{LLF}_{max}\) with the equality holding in the limit \(\mu \rightarrow \infty\). The reason is that the ROC gives credit for incorrect localizations on diseased cases, while the AFROC does not. This is the well-known ``right for wrong reason'' argument \citep{bunch1977free}, originally advanced in 1977, against usage of the ROC for localization tasks.
\item
  As \(\mu\) increases the AFROC curve more closely approaches the upper-left corner of the plot and the area under the AFROC curve approaches 1, which is the best possible performance: (a) \(\text{FPF}_{\text{max}}\) decreases and (b) \(\text{LLF}_{\text{max}}\) increases, meaning decreasing numbers of incorrect decisions on non-diseased and diseased cases, respectively.
\item
...
\item
  As \(\lambda \to 0\) decreases \(\text{FPF}_{\text{max}} \to 0\) while \(\text{LLF}_{\text{max}}\) stays constant, as the latter is independent of \(\lambda\).
\item
...
\end{aligned}
\right \} 
\label{eq:rsm-pred2-afroc-slope4}
\end{equation}

\hypertarget{wafroc-plots-using-physical-lambda-nu-parameters}{%
\subsection{\texorpdfstring{wAFROC plots using physical \(\lambda, \nu\) parameters}{wAFROC plots using physical \textbackslash lambda, \textbackslash nu parameters}}\label{wafroc-plots-using-physical-lambda-nu-parameters}}

\begin{figure}
\centering
\includegraphics{08-rsm-predictions2_files/figure-latex/rsm-pred2-wafroc-plots-lambda-nu-1.pdf}
\caption{\label{fig:rsm-pred2-wafroc-plots-lambda-nu}RSM-predicted wAFROC curves using \(\lambda, \nu\) paramterization. Top left: \(\mu = 0.0001\), \(\lambda = 1\) and \(\nu = 0.2\). Top right: \(\mu = 1\), \(\lambda = 2\) and \(\nu = 0.5\). Bottom left: \(\mu = 2\), \(\lambda = 3\) and \(\nu = 0.7\). Bottom right: \(\mu = 4\), \(\lambda = 4\) and \(\nu = 0.9\). As \(\mu\) increases the curve approaches the top-left corner. Each curve includes an inaccessible dashed linear extension to (1,1) and each plot is contained within the unit square which makes it easy to define the AUC as a figure of merit. Each wAFROC curve is slightly above the corresponding AFROC curve.}
\end{figure}

\hypertarget{wafroc-curves-are-generally-above-the-afroc-curves}{%
\subsection{wAFROC curves are generally above the AFROC curves}\label{wafroc-curves-are-generally-above-the-afroc-curves}}
...
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{) \{}
  \KeywordTok{cat}\NormalTok{(i, }\KeywordTok{all}\NormalTok{(plotArr_wAFROC_lambda_nu[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{wLLF }\OperatorTok{>=}\StringTok{ }\NormalTok{plotArr_AFROC_lambda_nu[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{LLF), }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
The \texttt{FALSE} value corresponds to \(\mu = 0.001\) and the last one corresponds to \(\mu = 4\).

\begin{quote}
If one sets \texttt{relWeights\ =\ 0} thereby ensuring equal weights of all lesions, as in the following code, the wAFROC and AFROC wLLF values become identical.
\end{quote}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plotArrwAfrocZero <-}\StringTok{ }\KeywordTok{array}\NormalTok{(}\KeywordTok{list}\NormalTok{(), }\DataTypeTok{dim =} \KeywordTok{c}\NormalTok{(}\KeywordTok{length}\NormalTok{(lambdaArr)))}
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\KeywordTok{length}\NormalTok{(lambdaArr)) \{}
\NormalTok{  mu <-}\StringTok{ }\NormalTok{muArr[i]}
\NormalTok{  lambda <-}\StringTok{ }\NormalTok{lambdaArr[i]}
\NormalTok{  nu <-}\StringTok{ }\NormalTok{nuArr[i]}
\NormalTok{  ret1 <-}\StringTok{ }\KeywordTok{PlotRsmOperatingCharacteristics}\NormalTok{(}
\NormalTok{    mu, lambda, nu,}
    \DataTypeTok{lesDistr =}\NormalTok{ lesDistr,}
    \DataTypeTok{relWeights =} \DecValTok{0}\NormalTok{,}
    \DataTypeTok{OpChType =} \StringTok{"wAFROC"}\NormalTok{,}
    \DataTypeTok{legendPosition  =} \StringTok{"none"}
\NormalTok{  )}
\NormalTok{  plotArrwAfrocZero[[i]] <-}\StringTok{ }\NormalTok{ret1}\OperatorTok{$}\NormalTok{wAFROCPlot}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{) \{}
  \KeywordTok{cat}\NormalTok{(i, }\KeywordTok{all}\NormalTok{(plotArrwAfrocZero[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{wLLF }\OperatorTok{<=}\StringTok{ }\NormalTok{plotArr_AFROC_lambda_nu[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{LLF), }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
Both tests \(\ge\) and \(\le\) yield \texttt{FALSE} implying they are equal (testing for equality does not yield \texttt{TRUE} due to numerical issues).
