...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{cat}\NormalTok{(}\KeywordTok{RSM_TPF}\NormalTok{(zeta,mu,lambda,nu, }\DataTypeTok{lesDistr =}\NormalTok{ lesDistr))}
\end{Highlighting}
\end{Shaded}
...
\chapter{How to analyze a dataset with only diseased cases}\label{analyze-diseased-only-dataset}}
...
\hypertarget{diseased-only-dataset}{%
\section{Handling diseased-only datasets}\label{diseased-only-dataset}}

ROC-like plot of TPF vs.~FPF1 is possible, see Section \ref{empirical-definition-empirical-auc-afroc1}. Can create a ROC-like dataset with equal number of ``non-diseased'' and diseased cases (the ratings of the non-diseased cases are the FP ratings on diseased cases). Fit RSM to this dataset. Proceed as before. Key assumption being violated: the FP ratings on diseased cases are independent of the TP ratings on same cases. However, without this assumption one cannot estimate RSM parameters. Need \texttt{RJafroc} function to handle this special case: \texttt{FitRsmRoc1}? No! Just need function to create a ``ROC'' dataset from one that only has diseased cases. e.g., FixK1eq0dataset?

Save TONY dataset to \texttt{dsTony}. Create copy \texttt{dsNoNormals}. Replace all marks with case indices greater than \(K_2\) with \texttt{NA}. This creates a dataset with no marks on non-diseased cases.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dsTony <-}\StringTok{ }\NormalTok{RJafroc}\OperatorTok{::}\NormalTok{dataset01 }\CommentTok{# TONY dataset}
\NormalTok{K2 <-}\StringTok{ }\KeywordTok{length}\NormalTok{(dsTony}\OperatorTok{$}\NormalTok{lesions}\OperatorTok{$}\NormalTok{perCase)}
\NormalTok{K1 <-}\StringTok{ }\KeywordTok{length}\NormalTok{(dsTony}\OperatorTok{$}\NormalTok{ratings}\OperatorTok{$}\NormalTok{NL[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,,}\DecValTok{1}\NormalTok{]) }\OperatorTok{-}\StringTok{ }\NormalTok{K2}
\NormalTok{I <-}\StringTok{ }\KeywordTok{length}\NormalTok{(dsTony}\OperatorTok{$}\NormalTok{ratings}\OperatorTok{$}\NormalTok{NL[,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{]) }\OperatorTok{-}\StringTok{ }\NormalTok{K2}
\NormalTok{J <-}\StringTok{ }\KeywordTok{length}\NormalTok{(dsTony}\OperatorTok{$}\NormalTok{ratings}\OperatorTok{$}\NormalTok{NL[}\DecValTok{1}\NormalTok{,,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{]) }\OperatorTok{-}\StringTok{ }\NormalTok{K2}
\NormalTok{dsNoNormals <-}\StringTok{ }\NormalTok{dsTony}
\NormalTok{dsNoNormals}\OperatorTok{$}\NormalTok{ratings}\OperatorTok{$}\NormalTok{NL <-}\StringTok{ }\NormalTok{dsNoNormals}\OperatorTok{$}\NormalTok{ratings}\OperatorTok{$}\NormalTok{NL[,,}\OperatorTok{-}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{K1),]}
\NormalTok{RJafroc}\OperatorTok{::}\KeywordTok{UtilFigureOfMerit}\NormalTok{(dsTony,}\DataTypeTok{FOM =} \StringTok{"wAFROC"}\NormalTok{)}
\CommentTok{#>            rdr1      rdr2      rdr3      rdr4      rdr5}
\CommentTok{#> trtBT 0.7602704 0.8406191 0.8171524 0.8153090 0.8278324}
\CommentTok{#> trtDM 0.6425854 0.7049977 0.7518434 0.7724426 0.6836962}
\CommentTok{#RJafroc::UtilFigureOfMerit(dsNoNormals,FOM = "wAFROC")}
\NormalTok{RJafroc}\OperatorTok{::}\KeywordTok{UtilFigureOfMerit}\NormalTok{(dsTony,}\DataTypeTok{FOM =} \StringTok{"wAFROC1"}\NormalTok{)}
\CommentTok{#>            rdr1      rdr2      rdr3      rdr4      rdr5}
\CommentTok{#> trtBT 0.8079866 0.8696629 0.8747798 0.8517613 0.8563468}
\CommentTok{#> trtDM 0.7277103 0.7781506 0.8225630 0.7968418 0.7496963}
\NormalTok{RJafroc}\OperatorTok{::}\KeywordTok{UtilFigureOfMerit}\NormalTok{(dsNoNormals,}\DataTypeTok{FOM =} \StringTok{"wAFROC1"}\NormalTok{)}
\CommentTok{#>            rdr1      rdr2      rdr3      rdr4      rdr5}
\CommentTok{#> trtBT 0.8594559 0.9009910 0.9369398 0.8910807 0.8871039}
\CommentTok{#> trtDM 0.8195304 0.8570572 0.8988448 0.8231600 0.8208875}
\end{Highlighting}
\end{Shaded}
