...
Both RSM and CBM are implemented in \texttt{RJafroc}. \texttt{PROPROC} is implemented in Windows software OR DBM-MRMC 2.5 \footnote{Sept.~04, 2014; the version used in this chapter is no longer distributed but is available from me upon request.} that was available \href{https://perception.lab.uiowa.edu/software}{here}, last accessed 1/4/21.
...
\item
  In general the values are accessed as \texttt{resultsVD{[}{[}i,j{]}{]}}, where \texttt{i} is the treatment index and \texttt{j} is the reader index;
\end{itemize}
...
\item
  \texttt{rresults{[}{[}i,j{]}{]}\$retCbm\$NLLFin)} is the final negative log-likelihood value.
\end{itemize}
