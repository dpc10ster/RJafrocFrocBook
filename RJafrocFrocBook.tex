...
\item
  \(\text{pmf}_{Bin}\left ( l_2, L, \nu \right )\) is the probability mass function (pmf) of the binomial distribution with success probability \(\nu\) and trial size \(L\), defined in Eqn. \eqref{eq:rsm-binomial-pmf}. \(\text{W}_{Ll_2}\) is the weight of lesion \(l_2\) in cases with \(L\) lesions; for example \(\text{W}_{42} = 0.3\).
\item
...
\section{TBA Discussion / Summary}\label{rsm-pred2-discussion-summary}}
