...
\caption{Help page for `RJafroc` function `UtilAucBormal`.}\label{fig:rsm-pred-binorml-help}
\end{figure}
...
The coordinates of the RSM-predicted operating point on the FROC curve for threshold \(\zeta\) are given by Eqn. \eqref{eq:rsm-pred2-nlf} and Eqn. \eqref{eq:rsm-pred2-llf}. The FROC curve starts at (0,0) and ends at \(\left ( \lambda, \nu \right )\) -- the end-point. The end-point is defined by:
...
The end-point is not constrained to lie within the unit-square, rather it is \emph{semi-constrained}: while the ordinate cannot exceed unity the abscissa can.
...
The following code shows how to generate FROC plots using the intrinsic \(\lambda_i = 2\) and \(\nu_i = 0.5\) parameters for 4 values of \(\mu\) contained in the array \texttt{muArr\ \textless{}-\ c(0.01,1,2,4)} (to avoid a divide by zero error the value \(\mu=0\) is not allowed). A \texttt{list} array \texttt{p\_FROC\_lambdai\_nui} \footnote{Notation: \texttt{p\_} stands for a plot array, \texttt{FROC} stands for type of plot (also allowed are \texttt{AFROC} and \texttt{wAFROC}), \texttt{lambdai} stands for \(\lambda_i\) and \texttt{nui} stands for \(\nu_i\) (also allowed are \texttt{lambda} for \(\lambda\) and \texttt{nu} for \(\nu\) ).} is created to hold the four plots. The intrinsic intrinsic \(\lambda_i, \nu_i\) parameters are converted to \(\lambda, \nu\) using the function \texttt{UtilIntrinsic2RSM()}which implements Eqn. \eqref{eq:rsm-pred2-transform}). The parameters are printed using the \texttt{cat()} function. The plots are generated using \texttt{PlotRsmOperatingCharacteristics()}. Online help on this function is \href{https://dpc10ster.github.io/RJafroc/reference/PlotRsmOperatingCharacteristics.html}{available}. The code \texttt{p\_FROC\_lambdai\_nui{[}{[}i{]}{]}\ \textless{}-\ ret1\$FROCPlot} saves the plot to the previously created \texttt{list} array.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{muArr <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.01}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{4}\NormalTok{)}
...
\caption{\label{fig:rsm-pred2-froc-plots-lambdai-nui}RSM-predicted FROC curves using intrinsic parameters \(\lambda_i = 2\) and \(\nu_i = 0.5\). Top left: \(\mu = 0.01\); Top right: \(\mu = 1\); Bottom left: \(\mu = 2\); Bottom right: \(\mu = 4\). Each plot is \textbf{not} contained within the unit square which makes it impossible to define the FROC-AUC as a figure of merit.}
\end{figure}
...
The AFROC x-coordinate is the same as the ROC x-coordinate and Eqn. \eqref{eq:rsm-pred-fpf} applies. The AFROC y-coordinate is identical to the FROC y-coordinate and Eqn. \eqref{eq:rsm-pred2-llf} applies. Therefore:
...
This equation is implemented in \texttt{UtilAnalyticalAucsRSM}. Help is available \href{https://dpc10ster.github.io/RJafroc/reference/UtilAnalyticalAucsRSM.html}{here}.

The binomial probability mass function \(\text{PMF}_{B}\) was defined in Eqn. \eqref{eq:rsm-binomial-PMF}. For convenience it is repeated here:

\begin{equation} 
\text{PMF}_{B}\left ( l_2, L, \nu \right ) = \binom{L}{l_2} \left (\nu  \right )^{l_2} \left (1-\nu  \right )^{L-l_2}
\label{eq:rsm-pred2-PMF-binomial}
\end{equation}
...
\item
  \(\mathbf{W}\) is the (lower triangular) square matrix with \(L_{max}\) rows and columns containing the weights, where each row (excluding cells set to \(-\infty\)) sums to unity, see example below (the unused matrix elements are set to \(-\infty\)). The relative lesion weights are denoted in the code \texttt{relWeights}. For example, \texttt{relWeights\ =\ \ c(0.2,\ 0.3,\ 0.1,\ 0.5)} whose meaning is as follows:
...
\hypertarget{rsm-pred2-appendix}{%
\section{Appendix}\label{rsm-pred2-appendix}}
...
\hypertarget{rsm-pred2-froc-physical-parameters}{%
\subsection{Slope of the FROC curve}\label{rsm-pred2-froc-physical-parameters}}
...
\end{aligned}
\right \} 
\label{eq:rsm-pred2-froc-slope1}
\end{equation}
...
\end{aligned}
\right \} 
\label{eq:rsm-pred2-froc-slope2}
\end{equation}
...
\end{aligned}
\right \} 
\label{eq:rsm-pred2-froc-slope3}
\end{equation}

Eqn. \eqref{eq:rsm-pred2-froc-slope3} leads to the following conclusions (recall \(\mu \ge 0\)):
...
If instead we had used Eqn. \eqref{eq:rsm-pred2-froc-slope2} and regarded \(\lambda\) and \(\nu\) as independent of \(\mu\), the last conclusion would change to:
...
\hypertarget{rsm-pred2-afroc-physical-parameters}{%
\subsection{Slope of the AFROC curve}\label{rsm-pred2-afroc-physical-parameters}}
...
Using Eqn. \eqref{eq:rsm-pred2-froc-slope1} the slope of the AFROC can be expressed in terms of the slope of the FROC:
...
\hypertarget{rsm-pred2-wafroc-physical-parameters}{%
\subsection{\texorpdfstring{wAFROC plots using physical \(\lambda, \nu\) parameters}{wAFROC plots using physical \textbackslash lambda, \textbackslash nu parameters}}\label{rsm-pred2-wafroc-physical-parameters}}
...
\hypertarget{rsm-pred2-wafroc-above-afroc}{%
\subsection{wAFROC curves are generally above the AFROC curves}\label{rsm-pred2-wafroc-above-afroc}}
...
The following code compares wAFROC \texttt{wLLF} values to corresponding AFROC \texttt{LLF} values. If all of the former are greater than or equal to all of the latter, the \texttt{all()} function yields \texttt{TRUE}.

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{) \{}
  \KeywordTok{cat}\NormalTok{(i, }\KeywordTok{all}\NormalTok{(p_wAFROC_lambda_nu[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{wLLF }\OperatorTok{>=}\StringTok{ }\NormalTok{p_AFROC_lambda_nu[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{LLF), }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
The \texttt{FALSE} value corresponds to \(\mu = 0.01\), likely a numerical issue associated with commparing very small values. The remaining \(\mu\) values yield \texttt{TRUE}.

\begin{quote}
If one sets \texttt{relWeights\ =\ 0} thereby ensuring equal weights of all lesions, as in the following code, the AFROC-LLF and wAFROC-wLLF values become identical:
\end{quote}

In the following code we set \texttt{relWeights\ =\ 0} to ensure equal weighting of all lesions.
...
For equal weights the following code tests if wAFROC \texttt{wLLF} values are less than or equal to the corresponding AFROC \texttt{LLF} values. (In \texttt{plotArrwAfrocZero} the Zero refers to zero weights).

\begin{Shaded}
\begin{Highlighting}[]
\ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{4}\NormalTok{) \{}
  \KeywordTok{cat}\NormalTok{(i, }\KeywordTok{all}\NormalTok{(plotArrwAfrocZero[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{wLLF }\OperatorTok{<=}\StringTok{ }\NormalTok{p_AFROC_lambda_nu[[i]]}\OperatorTok{$}\NormalTok{data}\OperatorTok{$}\NormalTok{LLF), }\StringTok{"}\CharTok{\textbackslash{}n}\StringTok{"}\NormalTok{)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
For equal weights the following code tests if wAFROC \texttt{wLLF} values are greaater than or equal to the corresponding AFROC \texttt{LLF} values.
...
Both tests yield \texttt{FALSE} implying the arrays are equal (testing for strict equality does not yield \texttt{TRUE} due to numerical issues).
